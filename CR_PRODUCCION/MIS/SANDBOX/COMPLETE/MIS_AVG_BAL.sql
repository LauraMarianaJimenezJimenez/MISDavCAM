...
SELECT A.COD_CONT, A.IDF_CTO, A.COD_GL, A.DES_GL, A.COD_ACCO_CENT, A.COD_OFFI, A.COD_BLCE_STATUS, A.COD_VALUE, 
A.COD_CURRENCY, A.COD_ENTITY, A.COD_PRODUCT, A.COD_SUBPRODUCT, A.COD_ACT_TYPE, A.EOPBAL_CAP, A.EOPBAL_INT,
A.AVGBAL_CAP, A.AVGBAL_INT, A.PL1, CONCAT(A.COD_INFO_SOURCE,IF(STRRIGHT(A.COD_INFO_SOURCE, 2)!='_M', '_M', ''))
FROM
    (SELECT COD_CONT, IDF_CTO, COD_GL, DES_GL, COD_ACCO_CENT, COD_OFFI, COD_BLCE_STATUS, COD_VALUE, 
    COD_CURRENCY, COD_ENTITY, COD_PRODUCT, COD_SUBPRODUCT, NULL AS COD_ACT_TYPE, 0 AS EOPBAL_CAP, 0 AS EOPBAL_INT,
    ...
    GROUP BY COD_CONT, IDF_CTO, COD_GL, DES_GL, COD_ACCO_CENT, COD_OFFI, COD_BLCE_STATUS, COD_VALUE, 
    COD_CURRENCY, COD_ENTITY, COD_PRODUCT, COD_SUBPRODUCT, COD_INFO_SOURCE) A
LEFT JOIN
    (SELECT DISTINCT COD_CONT, IDF_CTO, COD_GL, DES_GL, COD_ACCO_CENT, COD_OFFI, COD_BLCE_STATUS, COD_VALUE, 
    COD_CURRENCY, COD_ENTITY, COD_PRODUCT, COD_SUBPRODUCT, NULL AS COD_ACT_TYPE, COD_INFO_SOURCE
    FROM MIS_APR_${var:tabla}
    WHERE DATA_DATE = '${var:periodo}') B
ON A.IDF_CTO = B.IDF_CTO AND A.COD_BLCE_STATUS = B.COD_BLCE_STATUS AND A.COD_VALUE = B.COD_VALUE 
AND A.COD_PRODUCT = B.COD_PRODUCT AND A.COD_SUBPRODUCT = B.COD_SUBPRODUCT AND A.COD_CONT = B.COD_CONT 
AND A.COD_ENTITY = B.COD_ENTITY
...
CASE 
    WHEN a.COD_CURRENCY='CRC' THEN IFNULL(a.EOPBAL_CAP, 0)
    ELSE CAST(IFNULL(a.EOPBAL_CAP * b.EXCH_RATE, 0) AS DECIMAL(30,10)) END AS EOPBAL_CAP_EXCH,
CASE 
    WHEN a.COD_CURRENCY='CRC' THEN IFNULL(a.EOPBAL_INT, 0)
...
WHERE a.DATA_DATE = '${var:periodo}' AND a.COD_VALUE IN ('CAP','INT');
...
SELECT a.IDF_CTO, a.COD_BLCE_STATUS, a.COD_VALUE, a.COD_PRODUCT, a.COD_SUBPRODUCT, a.COD_ACCO_CENT, 'A' AS COD_INFO_SOURCE,
...
WHERE Year(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) =  Year(TO_TIMESTAMP('${var:periodo}','yyyyMMdd'))
    AND Month(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = Month(TO_TIMESTAMP('${var:periodo}','yyyyMMdd'))
    AND a.DATA_DATE <= '${var:periodo}'
    AND a.COD_VALUE <> 'RSL'  
    GROUP BY a.IDF_CTO, a.COD_BLCE_STATUS, a.COD_VALUE, a.COD_PRODUCT, a.COD_SUBPRODUCT, a.COD_ACCO_CENT, a.COD_CONT, a.COD_ENTITY;
...
LEFT JOIN -- (SELECT * FROM MIS_TMP_AVGBAL_${var:tabla}_EXCH WHERE DATA_DATE = '${var:periodo}') b
MIS_TMP_AVGBAL_${var:tabla}_EXCH b
ON a.IDF_CTO = b.IDF_CTO AND a.COD_BLCE_STATUS = b.COD_BLCE_STATUS AND a.COD_VALUE = b.COD_VALUE AND a.COD_PRODUCT = b.COD_PRODUCT 
AND a.COD_SUBPRODUCT = b.COD_SUBPRODUCT AND ISNULL(a.COD_ACCO_CENT,'0') = ISNULL(b.COD_ACCO_CENT,'0')
...
TRUNCATE TABLE IF EXISTS MIS_TMP_LOAD_AVGBAL_${var:tabla};
DROP TABLE IF EXISTS MIS_TMP_LOAD_AVGBAL_${var:tabla};
CREATE TABLE MIS_TMP_LOAD_AVGBAL_${var:tabla}
(IDF_CTO STRING, COD_BLCE_STATUS STRING,
COD_VALUE STRING, COD_PRODUCT STRING, COD_SUBPRODUCT STRING, COD_INFO_SOURCE STRING,
AVGBAL_CAP DECIMAL(30,10), AVGBAL_INT DECIMAL(30,10),
COD_CONT STRING, COD_ENTITY STRING);
...
INSERT INTO MIS_TMP_LOAD_AVGBAL_${var:tabla}
    ...
    AND COD_VALUE <> 'RSL'  
    GROUP BY IDF_CTO, COD_BLCE_STATUS, COD_VALUE, COD_PRODUCT, COD_SUBPRODUCT, COD_CONT, COD_ENTITY;

--COMPUTE STATS MIS_TMP_LOAD_AVGBAL_${var:tabla};
...
SELECT A.COD_CONT, A.IDF_CTO, A.COD_GL, A.DES_GL, A.COD_ACCO_CENT, A.COD_OFFI, A.COD_BLCE_STATUS, A.COD_VALUE, 
A.COD_CURRENCY, A.COD_ENTITY, A.COD_PRODUCT, A.COD_SUBPRODUCT, A.COD_ACT_TYPE, A.EOPBAL_CAP, 
...
ON (A.IDF_CTO = B.IDF_CTO AND A.COD_BLCE_STATUS = B.COD_BLCE_STATUS AND A.COD_VALUE = B.COD_VALUE AND 
A.COD_PRODUCT = B.COD_PRODUCT AND A.COD_SUBPRODUCT = B.COD_SUBPRODUCT AND A.COD_ENTITY = B.COD_ENTITY)  
WHERE (A.DATA_DATE = '${var:periodo}');
...
INSERT INTO MIS_APR_CONTRACT_DT PARTITION(DATA_DATE, COD_INFO_SOURCE)
    SELECT DISTINCT DT_ARR.IDF_CTO_ARR, COD_ENTITY, COD_PRODUCT, COD_SUBPRODUCT, COD_ACT_TYPE, COD_CURRENCY, IDF_CLI, COD_ACCO_CENT, COD_OFFI, COD_BCA_INT, COD_AMRT_MET, COD_RATE_TYPE, RATE_INT,
    DATE_ORIGIN, DATE_LAST_REV, DATE_PRX_REV, EXP_DATE, FREQ_INT_PAY, COD_UNI_FREQ_INT_PAY, FRE_REV_INT, COD_UNI_FRE_REV_INT, AMRT_TRM, COD_UNI_AMRT_TRM, INI_AM, CUO_AM, CREDIT_LIM_AM,
    PREDEF_RATE_IND, PREDEF_RATE, IND_CHANNEL, COD_TYP_LIC, COU_CAR_OFF, COD_CONV, COD_EXEC_CTO, COD_COVID_PORT, DATE_DISB, CARD_NUMBER, 
    FIELD1_CTO, FIELD2_CTO, FIELD3_CTO, FIELD4_CTO, FIELD5_CTO, FIELD6_CTO, FIELD7_CTO, FIELD8_CTO, FIELD9_CTO, FIELD10_CTO,
    COD_PROG_CARD, DES_PROG_CARD, '${var:periodo}' AS DATA_DATE, COD_INFO_SOURCE 
    ...
    INNER JOIN 
        (SELECT ARR.FEC_MAX, ARR.IDF_CTO AS IDF_CTO_ARR FROM 
            ...
            FROM MIS_APR_${var:tabla} 
            WHERE DATA_DATE = '${var:periodo}'
            GROUP BY IDF_CTO,COD_PRODUCT, COD_SUBPRODUCT 
            HAVING SUM(EOPBAL_CAP) <> 0 OR SUM(AVGBAL_CAP) <> 0 OR SUM(PL) <> 0) AS B 
...
ON SUBSTR(A.DATA_DATE, 1, 6) = B.DATA_DATE AND A.IDF_CTO = B.IDF_CTO AND A.COD_VALUE = B.COD_VALUE AND 
A.COD_PRODUCT = B.COD_PRODUCT AND A.COD_SUBPRODUCT = B.COD_SUBPRODUCT AND A.COD_CURRENCY = B.COD_CURRENCY 
AND A.COD_BLCE_STATUS = B.COD_BLCE_STATUS AND A.COD_ENTITY = B.COD_ENTITY
;
*/
