...
PARTITION (DATA_DATE='${var:periodo}')
    ...
    FROM 
        ...
        WHERE YEAR(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = YEAR(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd')) AND MONTH(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = MONTH(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd')) 
        AND DATA_DATE < '${var:periodo}'    AND (TO_TIMESTAMP(DATA_DATE,'yyyyMMdd') = DAYS_SUB(TRUNC(TO_TIMESTAMP('${var:periodo}','yyyyMMdd'),'DD'),1))
...
INSERT INTO MIS_TMP_LOAD_AVGBAL_${var:tabla}
    ...
    AND COD_VALUE <> 'RSL'  
    GROUP BY IDF_CTO, COD_VALUE, --COD_PRODUCT, COD_SUBPRODUCT, 
    COD_CONT;


------------ Inserción en la tabla APR_${var:tabla} ------------
...
SELECT A.COD_CONT, A.IDF_CTO, A.COD_GL, A.DES_GL, A.COD_ACCO_CENT, A.COD_OFFI, A.COD_BLCE_STATUS, A.COD_VALUE, A.COD_CURRENCY, A.COD_ENTITY, A.COD_PRODUCT, A.COD_SUBPRODUCT, A.COD_ACT_TYPE, A.EOPBAL_CAP, 
...
ON (A.IDF_CTO = B.IDF_CTO AND A.COD_VALUE = B.COD_VALUE --AND A.COD_PRODUCT = B.COD_PRODUCT AND A.COD_SUBPRODUCT = B.COD_SUBPRODUCT
)  
    ...
    INNER JOIN 
        (SELECT ARR.FEC_MAX, ARR.IDF_CTO AS IDF_CTO_ARR FROM 
            (SELECT ANT.*  FROM 
                ...
                WHERE DATA_DATE < '${var:periodo}' --TO_TIMESTAMP(DATA_DATE, 'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP('${var:periodo}','yyyyMMdd'), 1) 
                AND YEAR(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = YEAR(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd')) 
                AND MONTH(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = MONTH(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'))
                GROUP BY IDF_CTO
                )  AS ANT 
                LEFT JOIN 
                (SELECT DISTINCT IDF_CTO AS IDF_CTO2
                FROM MIS_APR_CONTRACT_DT 
...
INSERT INTO MIS_APR_CONTRACT_DT PARTITION(DATA_DATE, COD_INFO_SOURCE)
WITH
APR_LAST_DATE AS ( --Identificar última vez que contrato aparece cargado
    SELECT IDF_CTO, MAX(DATA_DATE) AS FEC_MAX
    FROM MIS_APR_CONTRACT_DT
    WHERE DATA_DATE < '${var:periodo}'
    AND YEAR(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = YEAR(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'))
    AND MONTH(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = MONTH(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'))
    AND IF(strright(COD_INFO_SOURCE, 2)='_M', COD_INFO_SOURCE, CONCAT(COD_INFO_SOURCE,'_M')) IN 
           (SELECT DISTINCT IF(strright(COD_INFO_SOURCE, 2)='_M', COD_INFO_SOURCE, CONCAT(COD_INFO_SOURCE,'_M')) FROM MIS_APR_${var:tabla}) 
    GROUP BY IDF_CTO),
APR_TODAY AS ( --Identificar contratos cargados hoy
    SELECT DISTINCT IDF_CTO AS IDF_CTO2
    FROM MIS_APR_CONTRACT_DT
    WHERE DATA_DATE = '${var:periodo}'
       AND IF(strright(COD_INFO_SOURCE, 2)='_M', COD_INFO_SOURCE, CONCAT(COD_INFO_SOURCE,'_M')) IN 
           (SELECT DISTINCT IF(strright(COD_INFO_SOURCE, 2)='_M', COD_INFO_SOURCE, CONCAT(COD_INFO_SOURCE,'_M')) FROM MIS_APR_${var:tabla})  ),
APR_CUR_TABLE AS ( --Identificar contratos que hacen parte del flujo actual
    SELECT IDF_CTO
    FROM MIS_APR_${var:tabla}
    WHERE DATA_DATE = '${var:periodo}'
    GROUP BY IDF_CTO
    HAVING SUM(EOPBAL_CAP) <> 0 OR SUM(AVGBAL_CAP) <> 0 OR SUM(PL) <> 0
),
APR_NOT_TODAY AS ( --Filtrar por contratos del flujo actual y excluir los cargados hoy
    SELECT DISTINCT ANT.IDF_CTO AS IDF_CTO_ARR, ANT.FEC_MAX
    FROM APR_LAST_DATE ANT
    LEFT JOIN APR_TODAY NUE
    ON ANT.IDF_CTO = NUE.IDF_CTO2
    INNER JOIN APR_CUR_TABLE DT_ARR
    ON ANT.IDF_CTO = DT_ARR.IDF_CTO
    WHERE NUE.IDF_CTO2 IS NULL
),
APR_LAST_DATE_TOT AS ( --Tomar contratos del mes vigente
    SELECT DT.*
    FROM MIS_APR_CONTRACT_DT DT
    WHERE DATA_DATE < '${var:periodo}'
    AND YEAR(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = YEAR(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'))
...
--Arrastrar contratos del mes vigente no cargados en la fecha actual
SELECT DISTINCT DT_ARR.IDF_CTO_ARR, COD_ENTITY, COD_PRODUCT, COD_SUBPRODUCT, COD_ACT_TYPE, COD_CURRENCY, IDF_CLI, COD_ACCO_CENT, COD_OFFI, COD_BCA_INT, COD_AMRT_MET, COD_RATE_TYPE, RATE_INT,
    DATE_ORIGIN, DATE_LAST_REV, DATE_PRX_REV, EXP_DATE, FREQ_INT_PAY, COD_UNI_FREQ_INT_PAY, FRE_REV_INT, COD_UNI_FRE_REV_INT, AMRT_TRM, COD_UNI_AMRT_TRM, INI_AM, CUO_AM, CREDIT_LIM_AM,
    PREDEF_RATE_IND, PREDEF_RATE, IND_CHANNEL, COD_TYP_LIC, '${var:periodo}' AS DATA_DATE, COD_INFO_SOURCE
FROM APR_LAST_DATE_TOT DT
INNER JOIN APR_NOT_TODAY DT_ARR
ON DT_ARR.IDF_CTO_ARR = DT.IDF_CTO AND DT_ARR.FEC_MAX = DT.DATA_DATE; */
...
----Cruce con APR_${var:tabla} para los saldos nuevos, respetando los existentes para capital
...
ON SUBSTR(A.DATA_DATE, 1, 6) = B.DATA_DATE AND A.IDF_CTO = B.IDF_CTO AND A.COD_VALUE = B.COD_VALUE
; */
