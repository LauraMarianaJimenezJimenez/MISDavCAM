...
FROM 
    ...
    LEFT JOIN (
        SELECT DWH.IDF_CTO, COUNT(*) CONTEO 
        FROM MIS_DWH_${var:tabla} DWH
        WHERE DWH.DATA_DATE = '${var:periodo}' AND DWH.COD_VALUE NOT IN ('RSL') AND DWH.FTP1 IS NULL AND DWH.COD_PRODUCT!='CUVENORE'
        GROUP BY DWH.IDF_CTO) CONT
    ON TMP.IDF_CTO = CONT.IDF_CTO
    WHERE TMP.DATA_DATE = '${var:periodo}' AND TMP.COD_VALUE NOT IN ('RSL') AND TMP.FTP1 IS NULL AND TMP.COD_PRODUCT!='CUVENORE'
...
FROM
    ...
    CASE WHEN DWH.COD_RATE_TYPE = 'A' AND DWH.DATE_ORIGIN = DWH.DATA_DATE THEN DWH.DATE_ORIGIN
        ...
        WHEN DWH.COD_RATE_TYPE = 'V' AND 
             TO_TIMESTAMP(DWH.DATE_LAST_REV,'yyyyMMdd') BETWEEN DATE_SUB(TO_TIMESTAMP(DWH.DATA_DATE, 'yyyyMMdd'), 3) AND TO_TIMESTAMP(DWH.DATA_DATE,'yyyyMMdd') 
             THEN DWH.DATE_LAST_REV
    ...
    WHERE DWH.FTP IS NULL 
        ...
        OR (DWH.COD_RATE_TYPE = 'V' AND 
            TO_TIMESTAMP(DWH.DATE_LAST_REV,'yyyyMMdd') BETWEEN DATE_SUB(TO_TIMESTAMP(DWH.DATA_DATE, 'yyyyMMdd'), 3) AND TO_TIMESTAMP(DWH.DATA_DATE,'yyyyMMdd'))
        OR (TO_TIMESTAMP(DWH.DATE_ORIGIN,'yyyyMMdd') BETWEEN DATE_SUB(TO_TIMESTAMP(DWH.DATA_DATE, 'yyyyMMdd'), 3) AND TO_TIMESTAMP(DWH.DATA_DATE,'yyyyMMdd'))
...
SELECT a.DATA_DATE, a.COD_CONT, a.IDF_CTO, a.COD_GL, a.DES_GL, a.COD_ACCO_CENT, a.COD_OFFI, a.COD_BLCE_STATUS, a.COD_VALUE, a.COD_CURRENCY, a.COD_ENTITY, 
    ...
    a.COD_BUSINESS_LINE, a.DES_BUSINESS_LINE, CAST(CASE WHEN a.COD_CONT = 'PLZ' AND a.EXP_DATE < a.DATA_DATE THEN 0 ELSE ISNULL(b.TTI,c.TTI) END AS DECIMAL(30,10)) AS FTP, a.FTP_RESULT, 'N' AS IND_POOL, 
    a.DATE_ORIGIN, a.EXP_DATE, a.DATE_LAST_REV, ISNULL(b.METHOD_TTI,c.METHOD_TTI) AS METHOD_TTI, a.COD_SEGMENT, a.COD_AMRT_MET
FROM (SELECT a.* FROM MIS_TMP_${var:tabla}_INI a WHERE a.FTP IS NULL) a 
LEFT JOIN (SELECT * FROM MIS_TMP_${var:tabla}_TTI_MA WHERE COD_AMRT_MET <> '') b 
...
LEFT JOIN (SELECT * FROM MIS_TMP_${var:tabla}_TTI_MA WHERE COD_AMRT_MET = '') c
ON a.COD_BLCE_PROD = c.COD_BLCE_PROD AND a.COD_CURRENCY = c.COD_CURRENCY AND a.COD_BLCE_STATUS = c.COD_BLCE_STATUS
    AND IF(TRIM(c.COD_BUSINESS_LINE)='', '', a.COD_BUSINESS_LINE) = TRIM(c.COD_BUSINESS_LINE)
    AND IF(TRIM(c.COD_SEGMENT)='', '', a.COD_SEGMENT) = TRIM(c.COD_SEGMENT)
WHERE b.COD_BLCE_PROD IS NOT NULL OR c.COD_BLCE_PROD IS NOT NULL;

--COMPUTE INCREMENTAL STATS MIS_TMP_${var:tabla}_MA; 
...
LEFT JOIN (
    SELECT * FROM MIS_APR_CONTRACT_DT TMP
    WHERE TO_TIMESTAMP(TMP.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'), 1)
) DT_PREV 
...
ON TO_TIMESTAMP(DM.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP(TTP.DATA_DATE, 'yyyyMMdd'), 1)
  ...
  --AND TTP.COD_BLCE_STATUS NOT IN ('3', '4') 
