...
export RUTA_ORIGEN=${ruta_origen_ajustes}
...
hadoop dfs -test -e $RUTA_ORIGEN/Ajustes_Manuales_Contables_Especiales_$YEAR$MONTH$DAY.csv
echo $?

if hdfs dfs -test -e $RUTA_ORIGEN/Ajustes_Manuales_Contables_Especiales_$YEAR$MONTH$DAY.csv ; then

hdfs dfs -rm -skipTrash $RUTA_DESTINO/Ajustes_Manuales_Contables_Especiales.csv
hdfs dfs -cp $RUTA_ORIGEN/Ajustes_Manuales_Contables_Especiales_$YEAR$MONTH$DAY.csv $RUTA_DESTINO/Ajustes_Manuales_Contables_Especiales.csv

else

hdfs dfs -rm -skipTrash $RUTA_DESTINO/Ajustes_Manuales_Contables_Especiales.csv
hdfs dfs -touchz $RUTA_DESTINO/Ajustes_Manuales_Contables_Especiales.csv
...
#TARJETAS ADICIONALES
hadoop dfs -test -e $RUTA_ORIGEN_ACT/TarAdicionales_$YEAR$MONTH.csv
echo $?

if hdfs dfs -test -e $RUTA_ORIGEN_ACT/TarAdicionales_$YEAR$MONTH.csv ; then

hdfs dfs -rm -skipTrash $RUTA_DESTINO/TAR_ADICIONALES.CSV
hdfs dfs -cp $RUTA_ORIGEN_ACT/TarAdicionales_$YEAR$MONTH.csv $RUTA_DESTINO/TAR_ADICIONALES.CSV

else

hdfs dfs -rm -skipTrash $RUTA_DESTINO/TAR_ADICIONALES.CSV
hdfs dfs -touchz $RUTA_DESTINO/TAR_ADICIONALES.CSV
