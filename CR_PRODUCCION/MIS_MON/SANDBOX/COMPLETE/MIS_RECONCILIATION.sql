...
ACCOUNT_CONCEPT STRING, EOPBAL_CAP decimal(30, 10), EOPBAL_INT decimal(30, 10), PL decimal(30, 10));
...
SELECT TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, 
    CAST(SUM(TMP.EOPBAL_CAP) AS decimal(30, 10)) AS EOPBAL_CAP, CAST(SUM(TMP.EOPBAL_INT) AS decimal(30, 10)) AS EOPBAL_INT, CAST(SUM(TMP.PL) AS decimal(30, 10)) AS PL 
...
GROUP BY TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT
UNION ALL
SELECT TMP.DATA_DATE, TMP.CONTENIDO, TMP.CENTRO_CONTABLE, TMP.DIVISA, TMP.ENTIDAD, TMP.AGRUPADOR_CONTABLE, CAT.DES_GL_GROUP, CAT.ACCOUNT_CONCEPT, 
    CAST(SUM(TMP.IMPORTE_MO) AS decimal(30, 10)) AS EOPBAL_CAP, 0 AS EOPBAL_INT, CAST(SUM(TMP.PL_MO) AS decimal(30, 10)) AS PL 
FROM MIS_DWH_MANUAL_ADJ_OPER TMP
LEFT JOIN MIS_PAR_CAT_CAF CAT
ON TMP.AGRUPADOR_CONTABLE = CAT.COD_GL_GROUP
WHERE TMP.DATA_DATE='${var:periodo}'
GROUP BY TMP.DATA_DATE, TMP.CONTENIDO, TMP.CENTRO_CONTABLE, TMP.DIVISA, TMP.ENTIDAD, TMP.AGRUPADOR_CONTABLE, CAT.DES_GL_GROUP,  CAT.ACCOUNT_CONCEPT
;
...
SELECT DWH.DATA_DATE, DWH.COD_CONT, DWH.COD_ACCO_CENT, 'CRC' AS COD_CURRENCY, 
    DWH.COD_ENTITY, DWH.COD_GL_GROUP, DWH.DES_GL_GROUP, DWH.ACCOUNT_CONCEPT, 
    CASE
        WHEN DWH.COD_CURRENCY='CRC' THEN DWH.EOPBAL_CAP  
        WHEN DWH.COD_CURRENCY='COP' THEN DWH.EOPBAL_CAP / RT.EXCH_RATE 
        ELSE DWH.EOPBAL_CAP * RT.EXCH_RATE 
        END AS EOPBAL_CAP, 
    CASE
        WHEN DWH.COD_CURRENCY='CRC' THEN DWH.EOPBAL_INT 
        WHEN DWH.COD_CURRENCY='COP' THEN DWH.EOPBAL_INT / RT.EXCH_RATE
        ELSE DWH.EOPBAL_INT * RT.EXCH_RATE 
        END AS EOPBAL_INT, 
    CASE 
        WHEN DWH.COD_CURRENCY='CRC' THEN DWH.PL 
        WHEN DWH.COD_CURRENCY='COP' THEN DWH.PL / RTX.EXCH_RATE
        ELSE DWH.PL * RTX.EXCH_RATE 
        END AS PL
...
LEFT JOIN MIS_PAR_EXCH_RATE RTX
ON DWH.DATA_DATE = RTX.DATA_DATE AND RTX.COD_CONT = 'TC_PROMEDIO' AND DWH.COD_CURRENCY = RTX.COD_CURRENCY
AND IF(ISNULL(TRIM(RTX.COD_ENTITY),'') = '' ,'1',DWH.COD_ENTITY) = IF(ISNULL(TRIM(RTX.COD_ENTITY),'') = '','1',RTX.COD_ENTITY);
...
--- Selecciona las variables del flujo contable u operacional, según corresponda ---         
    ...
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN CAST((CONT.TOT_EOPBAL_CAP - OPER.TOT_EOPBAL_CAP) AS decimal(30, 10)) 
         WHEN CONT.DATA_DATE IS NOT NULL THEN CAST((CONT.TOT_EOPBAL_CAP) AS decimal(30, 10))
         WHEN OPER.DATA_DATE IS NOT NULL THEN CAST((-1 * OPER.TOT_EOPBAL_CAP) AS decimal(30, 10)) END AS EOPBAL_CAP,  
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN CAST((CONT.TOT_EOPBAL_INT - OPER.TOT_EOPBAL_INT) AS decimal(30, 10)) 
         WHEN CONT.DATA_DATE IS NOT NULL THEN CAST((CONT.TOT_EOPBAL_INT) AS decimal(30, 10))
         WHEN OPER.DATA_DATE IS NOT NULL THEN CAST((-1 * OPER.TOT_EOPBAL_INT) AS decimal(30, 10)) END AS EOPBAL_INT,  
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN 0
         WHEN CONT.DATA_DATE IS NOT NULL THEN CAST(CONT.TOT_EOPBAL_CAP AS DECIMAL(30,10))
         WHEN OPER.DATA_DATE IS NOT NULL THEN 0 END AS AVGBAL_CAP,
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN 0
         WHEN CONT.DATA_DATE IS NOT NULL THEN CAST(CONT.TOT_EOPBAL_INT AS DECIMAL(30,10))
         WHEN OPER.DATA_DATE IS NOT NULL THEN 0 END AS AVGBAL_INT,
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN CAST((CONT.TOT_PL - OPER.TOT_PL) AS decimal(30, 10)) 
         WHEN CONT.DATA_DATE IS NOT NULL THEN CAST((CONT.TOT_PL) AS decimal(30, 10))
         WHEN OPER.DATA_DATE IS NOT NULL THEN CAST((-1 * OPER.TOT_PL) AS decimal(30, 10)) END AS PL, 
...
FROM (
    SELECT TMP.DATA_DATE, '0' AS COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, NULL AS DES_GL_GROUP,
        NULL AS ACCOUNT_CONCEPT, SUM(IFNULL(TMP.EOPBAL_CAP, 0) + IFNULL(TMP.EOPBAL_INT, 0)) AS TOT_EOPBAL_CAP, SUM(IFNULL(TMP.EOPBAL_INT, 0)) AS TOT_EOPBAL_INT, SUM(IFNULL(TMP.PL, 0)) AS TOT_PL 
...
FULL JOIN (
    SELECT ACC.DATA_DATE, ACC.COD_ACCO_CENT, ACC.COD_CURRENCY, ACC.COD_ENTITY, ACC.COD_GL_GROUP, ACC.DES_GL_GROUP, 
        ACC.ACCOUNT_CONCEPT,
        SUM(IFNULL(ACC.EOPBAL_CAP, 0)) AS TOT_EOPBAL_CAP, SUM(IFNULL(ACC.EOPBAL_INT, 0)) AS TOT_EOPBAL_INT, SUM(IFNULL(ACC.PL, 0)) AS TOT_PL
...
SELECT DWH.COD_ACCO_CENT, '0' AS COD_BLCE_STATUS, DWH.COD_CURRENCY, DWH.COD_ENTITY,
    ...
    DWH.EXP_TYPE, DWH.EXP_FAMILY, NULL,
