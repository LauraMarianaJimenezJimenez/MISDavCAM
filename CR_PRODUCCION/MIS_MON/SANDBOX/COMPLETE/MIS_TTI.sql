...
SELECT COD_CONT, IDF_CTO, COD_GL, DES_GL, COD_ACCO_CENT, COD_OFFI, 
    ...
    CAST(AVG(FTP1) AS decimal(30, 6)) FTP1, CAST(AVG(FTP2) AS decimal(30, 6)) FTP2, 
    IND_POOL, COD_INFO_SOURCE
FROM MIS_DWH_${var:tabla}
WHERE SUBSTR(DATA_DATE, 1, 6) = SUBSTR('${var:periodo}', 1, 6)
    AND IND_POOL = 'N' AND (FTP1 <> 0 OR FTP2 <> 0) AND COD_VALUE <> 'RSL'
...
DROP TABLE IF EXISTS MIS_TMP_${var:tabla}_DIA_M;
CREATE TABLE MIS_TMP_${var:tabla}_DIA_M AS
SELECT COD_CONT, IDF_CTO, COD_GL, DES_GL,/* COD_OFFI,*/
    COD_VALUE, COD_CURRENCY, COD_ENTITY,/* COD_ACT_TYPE,*/
...
GROUP BY COD_CONT, IDF_CTO, COD_GL, DES_GL,/* COD_OFFI,*/
    COD_VALUE, COD_CURRENCY, COD_ENTITY,/* COD_ACT_TYPE,*/ IND_POOL, COD_INFO_SOURCE
;
...
SELECT A.COD_CONT, A.IDF_CTO, A.COD_GL, A.DES_GL, B.COD_ACCO_CENT, B.COD_OFFI, B.COD_BLCE_STATUS, A.COD_VALUE, A.COD_CURRENCY, A.COD_ENTITY, 
    ...
    B.ACCOUNT_CONCEPT, B.COD_PL_ACC, B.DES_PL_ACC, B.COD_BLCE_PROD, B.DES_BLCE_PROD, B.COD_BUSINESS_LINE, B.DES_BUSINESS_LINE, 
    B.COD_METHOD1, A.FTP1, B.COD_METHOD2, A.FTP2, 
    B.FTP_RESULT, A.IND_POOL, B.DATA_DATE, B.COD_INFO_SOURCE
FROM MIS_TMP_${var:tabla}_DIA_M A
LEFT JOIN (SELECT * FROM MIS_DWH_${var:tabla} WHERE DATA_DATE = '${var:periodo}' AND IND_POOL = 'N' /*AND FTP_RESULT <> 0*/) B
...
SELECT TMP.COD_CONT, TMP.IDF_CTO, TMP.COD_GL, TMP.DES_GL, TMP.COD_ACCO_CENT, TMP.COD_OFFI, TMP.COD_BLCE_STATUS, 
    TMP.COD_VALUE, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_PRODUCT, TMP.COD_SUBPRODUCT, TMP.COD_ACT_TYPE, 
    CAST(0 AS DECIMAL(30,10)), CAST(0 AS DECIMAL(30,10)), CAST(0 AS DECIMAL(30,10)), 
    ...
    TMP.COD_BLCE_PROD, TMP.DES_BLCE_PROD, TMP.COD_BUSINESS_LINE, TMP.DES_BUSINESS_LINE, 
    TMP.COD_METHOD1, CAST(TMP.FTP1 AS DECIMAL(30,10)), 
    TMP.COD_METHOD2, CAST(TMP.FTP2 AS DECIMAL(30,10)), 
    TMP.FTP_RESULT, TMP.IND_POOL, 
...
INSERT INTO MIS_DWH_${var:tabla}_M PARTITION (DATA_DATE, COD_INFO_SOURCE)
SELECT DWH.COD_CONT, DWH.IDF_CTO, DWH.COD_GL, DWH.DES_GL, DWH.COD_ACCO_CENT, DWH.COD_OFFI, DWH.COD_BLCE_STATUS, 
    DWH.COD_VALUE, DWH.COD_CURRENCY, DWH.COD_ENTITY, DWH.COD_PRODUCT, DWH.COD_SUBPRODUCT, DWH.COD_ACT_TYPE, 
    0 AS EOPBAL_CAP, 0 AS EOPBAL_INT, 0 AS AVGBAL_CAP, 0 AS AVGBAL_INT, 0 AS PL, 
    DWH.COD_GL_GROUP, DWH.DES_GL_GROUP, DWH.ACCOUNT_CONCEPT, DWH.COD_PL_ACC, DWH.DES_PL_ACC, 
    DWH.COD_BLCE_PROD, DWH.DES_BLCE_PROD, 
    'UNIDAD_FONDEO' AS COD_BUSINESS_LINE, 'UNIDAD DE FONDEO' AS DES_BUSINESS_LINE, 
    DWH.COD_METHOD1, DWH.FTP1, DWH.COD_METHOD2, DWH.FTP2, 
    CAST(-1*DWH.FTP_RESULT AS decimal(30, 10)) AS FTP_RESULT, 'Y' AS IND_POOL, 
    DWH.DATA_DATE, DWH.COD_INFO_SOURCE   
