...
--COMPUTE INCREMENTAL STATS MIS_APR_${var:tabla}_M PARTITION (DATA_DATE = '${var:periodo}');
...
LEFT JOIN MIS_PAR_REL_CAF_ACC ACC
ON APR.COD_GL = ACC.COD_GL AND APR.COD_ENTITY = ACC.COD_ENTITY AND APR.COD_CURRENCY = ACC.COD_CURRENCY
LEFT JOIN MIS_PAR_CAT_CAF CAT
ON CAF.COD_GL_GROUP = CAT.COD_GL_GROUP
LEFT JOIN MIS_PAR_CAT_CAF CAT_PR
ON CAF_PR.COD_GL_GROUP = CAT_PR.COD_GL_GROUP
LEFT JOIN MIS_PAR_CAT_CAF CAT_ACC
ON ACC.COD_GL_GROUP = CAT_ACC.COD_GL_GROUP
LEFT JOIN MIS_PAR_CAT_CAF CAT_APR
ON APR.COD_GL = CAT_APR.COD_GL_GROUP
;

--COMPUTE INCREMENTAL STATS MIS_TMP_STG_${var:tabla}_M_1;

----Rechazo de registros sin agrupador contable
INSERT OVERWRITE MIS_REJECTIONS_DIMENSIONS_M (DIM_REJ, COD_ENTITY, COD_CURRENCY, COD_GL, COD_GL_GROUP, COD_ACT_TYPE, COD_PRODUCT, COD_SUBPRODUCT, COD_RATE_TYPE, COD_BLCE_STATUS, COD_VALUE, 
...
FROM MIS_TMP_STG_${var:tabla}_M_1 a 
...
TRUNCATE TABLE IF EXISTS MIS_TMP_STG_${var:tabla}_M_2;
DROP TABLE IF EXISTS MIS_TMP_STG_${var:tabla}_M_2;
CREATE TABLE MIS_TMP_STG_${var:tabla}_M_2 AS
SELECT SALIDA_CAF.*, 
...
FROM (
    SELECT TMP.* 
    FROM MIS_TMP_STG_${var:tabla}_M_1 TMP 
...
--COMPUTE INCREMENTAL STATS MIS_TMP_STG_${var:tabla}_M_2;
...
SELECT 'CUENTA DE GESTION' AS DIM_REJ, a.COD_ENTITY, a.COD_CURRENCY, 'N/A', a.COD_GL_GROUP, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A',  
    'N/A', 'N/A', 'N/A', 'N/A', 'N/A', CAST(SUM(IFNULL(a.EOPBAL_CAP, 0) + IFNULL(a.EOPBAL_INT, 0)) AS DECIMAL(30,10)) AS AMO_EOPBAL, CAST(SUM(a.PL) AS DECIMAL(30,10)) AS AMO_PL, a.DATA_DATE, a.COD_CONT 
FROM MIS_TMP_STG_${var:tabla}_M_2 a 
...
---------------------------------------------------------------------
TRUNCATE TABLE IF EXISTS MIS_TMP_STG_${var:tabla}_M_3;
DROP TABLE IF EXISTS MIS_TMP_STG_${var:tabla}_M_3;
CREATE TABLE MIS_TMP_STG_${var:tabla}_M_3 AS
SELECT SALIDA_CG.*, 
...
FROM (
    SELECT TMP.* 
    FROM MIS_TMP_STG_${var:tabla}_M_2 TMP 
...
--COMPUTE INCREMENTAL STATS MIS_TMP_STG_${var:tabla}_M_3;
...
SELECT 'PRODUCTO DE BALANCE' AS DIM_REJ, a.COD_ENTITY, a.COD_CURRENCY, 'N/A', a.COD_GL_GROUP, a.COD_ACT_TYPE, a.COD_PRODUCT, a.COD_SUBPRODUCT, 'N/A', 'N/A', 'N/A',   
    'N/A', 'N/A', 'N/A', 'N/A', 'N/A', CAST(SUM(IFNULL(a.EOPBAL_CAP, 0) + IFNULL(a.EOPBAL_INT, 0)) AS DECIMAL(30, 10)) AS AMO_EOPBAL, CAST(SUM(a.PL) AS DECIMAL(30, 10)) AS AMO_PL, a.DATA_DATE, a.COD_CONT 
FROM MIS_TMP_STG_${var:tabla}_M_3 a  
...
---------------------------------------------------
TRUNCATE TABLE IF EXISTS MIS_TMP_STG_${var:tabla}_M_4;
DROP TABLE IF EXISTS MIS_TMP_STG_${var:tabla}_M_4;
CREATE TABLE MIS_TMP_STG_${var:tabla}_M_4 AS
SELECT DWH.*, 
...
FROM (
    SELECT TMP.* 
    FROM MIS_TMP_STG_${var:tabla}_M_3 TMP 
...
--COMPUTE INCREMENTAL STATS MIS_TMP_STG_${var:tabla}_M_4;
...
FROM MIS_TMP_STG_${var:tabla}_M_4 a 
...
---Inserción de registros diarios en tabla mensual
INSERT INTO MIS_DWH_${var:tabla}_M PARTITION (DATA_DATE, COD_INFO_SOURCE)
SELECT *
...
--ALTER TABLE MIS_DWH_${var:tabla}_M ADD IF NOT EXISTS PARTITION (DATA_DATE='${var:periodo}', COD_INFO_SOURCE='NA');
