...
SELECT *
FROM MIS_DM_BALANCE_RESULT_M A
...
INSERT INTO MIS_DM_BALANCE_RESULT_M PARTITION(DATA_DATE, COD_INFO_SOURCE)
SELECT *
...
CREATE TABLE MIS_TMP_TAXES_PAR AS 
	SELECT a.COD_ENTITY, a.COD_GL_GROUP_DESTINY AS COD_GL_GROUP, a.COD_ACCO_CENT_DESTINY AS COD_ACCO_CENT, a.TAX_PERC, b.DES_GL_GROUP, b.ACCOUNT_CONCEPT, '0' AS COD_BLCE_STATUS, d.COD_PL_ACC, e.DES_PL_ACC, 
	f.COD_BLCE_PROD, g.DES_BLCE_PROD 
	FROM MIS_PAR_TAX_ENG a 
	LEFT JOIN MIS_PAR_CAT_CAF b 
	ON a.COD_GL_GROUP_DESTINY = b.COD_GL_GROUP AND b.COD_CURRENCY = 'CRC'
...
CREATE TABLE MIS_TMP_TAXES_PL AS
	...
	FROM 
		...
		FROM MIS_DM_BALANCE_RESULT_M a
                INNER JOIN (SELECT * FROM MIS_HIERARCHY_PL_ACC WHERE COD_LEVEL_01 NOT IN ('IMP_REN_ANO','NO_PYG')) b
                ON a.COD_PL_ACC = b.COD_LEVEL_08
		WHERE DATA_DATE='${var:periodo}' AND (PL_TOT_RPT<>0 OR FTP_RESULT_RPT<>0) AND UPPER(TRIM(a.COD_INFO_SOURCE)) NOT IN 
                (SELECT DISTINCT CONCAT('AJUSTE_',UPPER(CRITERIO_DE_AJUSTE)) FROM MIS_DWH_MANUAL_ADJ_SPE_ACC 
...
CREATE TABLE MIS_TMP_TAXES_CONS AS
	...
	CAST((-1 * IFNULL(a.PL, 0) * b.TAX_PERC) AS DECIMAL(30, 10)) AS IMPUESTO_PL
	FROM 
...
(COD_ACCO_CENT, COD_BLCE_STATUS, COD_CURRENCY, COD_ENTITY, EOPBAL_CAP, EOPBAL_INT, AVGBAL_CAP, AVGBAL_INT, PL, PL_TOT, PL_TOT_RPT, PL_RPT, COD_GL_GROUP, 
...
SELECT A.COD_ACCO_CENT, A.COD_BLCE_STATUS, 'CRC' AS COD_CURRENCY, A.COD_ENTITY, A.EOPBAL_CAP, A.EOPBAL_INT, A.AVGBAL_CAP, A.AVGBAL_INT, CAST(A.PL AS DECIMAL(30,10)) AS PL, CAST(A.PL AS DECIMAL(30,10)) AS PL_TOT, 
CAST(A.PL AS DECIMAL(30,10)) AS PL_TOT_RPT, CAST(A.PL AS DECIMAL(30,10)) AS PL_RPT, A.COD_GL_GROUP, 
...
NULL AS EXP_TYPE, NULL AS EXP_FAMILY, A.COD_CONT, A.DATA_DATE, A.COD_INFO_SOURCE
...
DES_GL_GROUP, ACCOUNT_CONCEPT, COD_PL_ACC, DES_PL_ACC, COD_BLCE_PROD, DES_BLCE_PROD, COD_BUSINESS_LINE, DES_BUSINESS_LINE, IND_POOL, COD_SEGMENT, DES_SEGMENT, EXP_TYPE, EXP_FAMILY, COD_CONT) 
PARTITION(DATA_DATE, COD_INFO_SOURCE)
SELECT A.COD_ACCO_CENT, A.COD_BLCE_STATUS, A.COD_CURRENCY, A.COD_ENTITY, CAST(A.EOPBAL_CAP AS DECIMAL(30,10)) AS EOPBAL_CAP, CAST(A.EOPBAL_INT AS DECIMAL(30,10)) AS EOPBAL_INT, CAST(A.AVGBAL_CAP AS DECIMAL(30,10)) AS AVGBAL_CAP, 
CAST(A.AVGBAL_INT AS DECIMAL(30,10)) AS AVGBAL_INT, CAST(A.PL AS DECIMAL(30,10)) AS PL, CAST(A.PL AS DECIMAL(30,10)) AS PL_TOT, CAST(A.PL AS DECIMAL(30,10)) AS PL_TOT_RPT, CAST(A.PL AS DECIMAL(30,10)) AS PL_RPT, A.COD_GL_GROUP, 
A.DES_GL_GROUP, A.ACCOUNT_CONCEPT, A.COD_PL_ACC, A.DES_PL_ACC, A.COD_BLCE_PROD, A.DES_BLCE_PROD, A.COD_BUSINESS_LINE, A.DES_BUSINESS_LINE, A.IND_POOL, A.COD_SEGMENT, A.DES_SEGMENT, 
NULL AS EXP_TYPE, NULL AS EXP_FAMILY, A.COD_CONT, A.DATA_DATE, A.COD_INFO_SOURCE
...
DES_GL_GROUP, ACCOUNT_CONCEPT, COD_PL_ACC, DES_PL_ACC, COD_BLCE_PROD, DES_BLCE_PROD, COD_BUSINESS_LINE, DES_BUSINESS_LINE, IND_POOL, COD_SEGMENT, DES_SEGMENT, EXP_TYPE, EXP_FAMILY, COD_CONT) 
PARTITION(DATA_DATE, COD_INFO_SOURCE)
SELECT COD_ACCO_CENT, COD_BLCE_STATUS, COD_CURRENCY, COD_ENTITY, EOPBAL_CAP, EOPBAL_INT, AVGBAL_CAP, AVGBAL_INT, CAST(-1*PL AS DECIMAL(30,10)) AS PL, CAST(-1*PL_TOT AS DECIMAL(30,10)) AS PL_TOT, 
CAST(-1*PL_TOT_RPT AS DECIMAL(30,10)) AS PL_TOT_RPT, CAST(-1*PL_RPT AS DECIMAL(30,10)) AS PL_RPT, COD_GL_GROUP, DES_GL_GROUP, ACCOUNT_CONCEPT, COD_PL_ACC, DES_PL_ACC, COD_BLCE_PROD, 
DES_BLCE_PROD, COD_BUSINESS_LINE, DES_BUSINESS_LINE, IND_POOL, COD_SEGMENT, DES_SEGMENT, NULL AS EXP_TYPE, NULL AS EXP_FAMILY, 'RCCL' AS COD_CONT, DATA_DATE, COD_INFO_SOURCE
