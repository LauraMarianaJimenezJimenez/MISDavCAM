...
SELECT COD_CONT, IDF_CTO, COD_GL, DES_GL, COD_OFFI, COD_VALUE, COD_CURRENCY, COD_ENTITY, COD_ACT_TYPE, CAST(AVG(FTP1) AS decimal(30, 6)) FTP1, CAST(AVG(FTP2) AS decimal(30, 6)) FTP2, IND_POOL, COD_INFO_SOURCE
FROM MIS_DWH_${var:tabla}
WHERE SUBSTR(DATA_DATE,1,6) = SUBSTR('${var:periodo}',1,6)
AND IND_POOL = 'N'
AND (FTP1 <> 0 OR FTP2 <> 0)
AND COD_VALUE <> 'RSL'
GROUP BY COD_CONT, IDF_CTO, COD_GL, DES_GL, COD_OFFI, COD_VALUE, COD_CURRENCY, COD_ENTITY, COD_ACT_TYPE, IND_POOL, COD_INFO_SOURCE
;
*/
...
DROP TABLE IF EXISTS MIS_TMP_${var:tabla}_DIA_M;
CREATE TABLE MIS_TMP_${var:tabla}_DIA_M AS
SELECT COD_CONT, IDF_CTO, COD_GL, DES_GL, 
...
GROUP BY COD_CONT, IDF_CTO, COD_GL, DES_GL, 
...
SELECT TMP.COD_CONT, TMP.IDF_CTO, TMP.COD_GL, TMP.DES_GL, TMP.COD_ACCO_CENT, TMP.COD_OFFI, TMP.COD_BLCE_STATUS, TMP.COD_VALUE, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_PRODUCT, TMP.COD_SUBPRODUCT, TMP.COD_ACT_TYPE, CAST(0 AS DECIMAL(30,10)) AS EOPBAL_CAP, CAST(0 AS DECIMAL(30,10)) AS EOPBAL_INT, CAST(0 AS DECIMAL(30,10)) AS AVGBAL_CAP, 
    CAST(0 AS DECIMAL(30,10)) AS AVGBAL_INT, CAST(0 AS DECIMAL(30,10)) AS PL, 
    TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, TMP.COD_PL_ACC, TMP.DES_PL_ACC, 
    TMP.COD_BLCE_PROD, TMP.DES_BLCE_PROD, 
    TMP.COD_BUSINESS_LINE, TMP.DES_BUSINESS_LINE, TMP.COD_METHOD1, CAST(TMP.FTP1 AS DECIMAL(30,10)) AS FTP1, TMP.COD_METHOD2, CAST(TMP.FTP2 AS DECIMAL(30,10)) AS FTP2, TMP.FTP_RESULT, TMP.IND_POOL, 
