...
COD_TYP_FEE,IDF_CLI,TYP_DOC_ID,NUM_DOC_ID,NOM_NAME,NOM_LASTN,COD_RES_COUNT,COD_TYP_CLNT,COD_COND_CLNT,
COD_ENG,VAL_SEX,COD_SEGMENT,DES_SEGMENT,
...
FTP_RESULT_RPT,
COD_GL_GROUP,DES_GL_GROUP,ACCOUNT_CONCEPT,COD_PL_ACC,DES_PL_ACC,COD_BLCE_PROD,DES_BLCE_PROD,COD_BUSINESS_LINE,DES_BUSINESS_LINE,FTP,FTP_RESULT,IND_POOL,COD_SUBSEGMENT,COD_GROUP_EC,EXP_TYPE,EXP_FAMILY)
...
SELECT 'RCTB', a.DIVISA, a.ENTIDAD, '', '', '', ISNULL(a.CENTRO_COSTO,''), '', '', '',  '', '', 
    ...
    CAST(IFNULL(A.IMPORTE_ML / l.EXCH_RATE, 0) AS DECIMAL(30,10)) AS EOPBAL_TOT_REG,
    CAST(IFNULL(A.SALDO_MEDIO_ML / j.EXCH_RATE, 0) AS DECIMAL(30,10)) AS AVGBAL_TOT_USD,
    CAST(IFNULL(A.SALDO_MEDIO_ML / l.EXCH_RATE, 0) AS DECIMAL(30,10)) AS AVGBAL_TOT_REG,  
    CASE    
        WHEN A.DIVISA='USD' THEN CAST(IFNULL(A.PL_ML,0) AS DECIMAL(30,10))
        ELSE CAST(IFNULL(A.PL_ML / k.EXCH_RATE,0) AS DECIMAL(30,10)) END AS PL_TOT_USD,
    CASE 
        WHEN A.DIVISA='USD' THEN CAST(IFNULL(A.PL_ML, 0) AS DECIMAL(30,10))
        ELSE CAST(IFNULL(A.PL_ML / m.EXCH_RATE, 0) AS DECIMAL(30,10)) END AS PL_TOT_REG,
    NULL, 
    ...
    END AS EXP_FAMILY, '${var:periodo}' AS DATA_DATE, CONCAT('AJUSTE_',a.CRITERIO_DE_AJUSTE) AS COD_INFO_SOURCE
FROM MIS_DWH_MANUAL_ADJ_ACC a 
LEFT JOIN MIS_PAR_REL_CAF_ACC b
...
ON TRIM(AL4.COD_EXPENSE) = '' AND a.CENTRO_COSTO=AL4.COD_ACCO_CENT AND a.AGRUPADOR_CONTABLE=AL4.COD_GL_GROUP 
LEFT JOIN MIS_PAR_REL_EXP_TYP AL6 
ON COALESCE(AL4.EXP_FAMILY) IS NULL 
   AND TRIM(AL6.COD_EXPENSE) = '' AND a.CENTRO_COSTO=AL6.COD_ACCO_CENT AND TRIM(AL6.COD_GL_GROUP) = '' 
LEFT JOIN MIS_PAR_REL_EXP_TYP AL7 
ON COALESCE(AL4.EXP_FAMILY, AL6.EXP_FAMILY) IS NULL 
   AND TRIM(AL7.COD_EXPENSE) = '' AND TRIM(AL7.COD_ACCO_CENT) = '' AND a.AGRUPADOR_CONTABLE=AL7.COD_GL_GROUP
...
ON a.DATA_DATE = k.DATA_DATE AND k.COD_CONT = 'TC_PROMEDIO' AND k.COD_CURRENCY = 'USD'
AND IF(ISNULL(TRIM(k.COD_ENTITY),'') = '' ,'1',a.ENTIDAD) = IF(ISNULL(TRIM(k.COD_ENTITY),'') = '','1',k.COD_ENTITY)
LEFT JOIN MIS_PAR_EXCH_RATE l
ON a.DATA_DATE = l.DATA_DATE AND l.COD_CONT = 'REG_FOTO' AND l.COD_CURRENCY = 'USD'
...
WHERE a.DATA_DATE='${var:periodo}' AND TRIM(UPPER(a.CONTENIDO)) = 'CONTABLE' AND TRIM(a.AGRUPADOR_CONTABLE) != '' AND TRIM(a.CUENTA_PYG) != '' AND TRIM(a.PRODUCTO_BALANCE) != '' AND TRIM(a.LINEA_NEGOCIO) != '' 
