...
CASE WHEN DM.IND_MENSUALIZA = 'Y' THEN
     ...
     ELSE CAST((MAX(ACT.PL) / PYG.EXCH_RATE) AS DECIMAL(30, 10)) END AS DECIMAL(30, 10)) 
ELSE
     CAST(CASE WHEN DM.COD_CURRENCY = 'USD' THEN (MAX(ISNULL(ACT.PL, 0))-MAX(ISNULL(PRV.PL, 0))) 
     ELSE (MAX(ISNULL(ACT.PL, 0))-MAX(ISNULL(PRV.PL, 0))) / PYG.EXCH_RATE END AS DECIMAL(30,10)) END AS PL_PPTO_USD,
CASE WHEN DM.IND_MENSUALIZA = 'Y' AND DM.COD_CURRENCY = 'USD' THEN CAST((SUM(ACU.PL)) AS DECIMAL(30, 10))
     ELSE CAST((MAX(ACU.PL) / PYG.EXCH_RATE) AS DECIMAL(30, 10)) END AS PL_PPTO_YTD_USD,  
CASE WHEN ACT.COD_CURRENCY = 'USD' THEN CAST(MAX(ACT.EOPBAL_CAP) AS DECIMAL(30,10))
     ELSE CAST((MAX(ACT.EOPBAL_CAP) / BLCE.EXCH_RATE) AS DECIMAL(30, 10)) END AS EOPBAL_TOT_USD,
CASE WHEN ACT.COD_CURRENCY = 'USD' THEN CAST(MAX(ACT.EOPBAL_CAP) AS DECIMAL(30,10))
     ELSE CAST((MAX(ACT.EOPBAL_CAP) / BLCE.EXCH_RATE) AS DECIMAL(30, 10)) END AS EOPBAL_TOT_REG,
CASE WHEN ACT.COD_CURRENCY = 'USD' THEN CAST(MAX(ACT.AVGBAL_CAP) AS DECIMAL(30,10))
     ELSE CAST((MAX(ACT.AVGBAL_CAP) / BLCE.EXCH_RATE) AS DECIMAL(30, 10)) END AS AVGBAL_TOT_USD,
CASE WHEN ACT.COD_CURRENCY = 'USD' THEN CAST(MAX(ACT.AVGBAL_CAP) AS DECIMAL(30,10))
     ELSE CAST((MAX(ACT.AVGBAL_CAP) / BLCE.EXCH_RATE) AS DECIMAL(30, 10)) END AS AVGBAL_TOT_REG,
CASE WHEN ACT.COD_CURRENCY = 'USD' THEN CAST(MAX(ACT.INI_AM) AS DECIMAL(30,10))
     ELSE CAST((MAX(ACT.INI_AM) / BLCE.EXCH_RATE) AS DECIMAL(30, 10)) END AS INI_AM_REG,
CASE WHEN DM.IND_MENSUALIZA = 'Y' THEN
     CAST(CASE WHEN DM.COD_CURRENCY = 'USD' THEN (MAX(ISNULL(ACT.PL, 0)))
     ELSE CAST((MAX(ACT.PL) / PYG.EXCH_RATE) AS DECIMAL(30, 10)) END AS DECIMAL (30,10))
ELSE 
     CAST(CASE WHEN DM.COD_CURRENCY = 'USD' THEN (MAX(ISNULL(ACT.PL, 0))-MAX(ISNULL(PRV.PL, 0))) 
     ELSE (MAX(ISNULL(ACT.PL, 0))-MAX(ISNULL(PRV.PL, 0))) / PYG.EXCH_RATE END AS DECIMAL(30,10)) END AS PL_PPTO_REG,
CASE WHEN DM.IND_MENSUALIZA = 'Y' AND DM.COD_CURRENCY = 'USD' THEN CAST((SUM(ACU.PL)) AS DECIMAL(30, 10))
     ELSE CAST((MAX(ACU.PL) / PYG.EXCH_RATE) AS DECIMAL(30, 10)) END AS PL_PPTO_YTD_REG,  
...
LEFT JOIN (
    ...
    ACT.COD_ENTITY, ACT.COD_SEGMENT, ACT.COD_ACCO_CENT, ACT.COD_EXPENSE, ACT.EXP_FAM, ACT.EXP_TYPE, 
    CAST(SUM(ACT.NUM_CTOS) AS INT) AS NUM_CTOS, ACT.ESCENARIO 
    FROM MIS_APR_BUDGET ACT
    GROUP BY ACT.DATA_DATE, ACT.COD_CONT, ACT.COD_GL_GROUP, ACT.COD_BUSINESS_LINE, ACT.COD_PL_ACC, ACT.COD_BLCE_PROD, 
    ACT.COD_CURRENCY, ACT.IND_MENSUALIZA, ACT.COD_ENTITY, ACT.COD_SEGMENT, ACT.COD_ACCO_CENT, 
    ACT.COD_EXPENSE, ACT.EXP_FAM, ACT.EXP_TYPE, ACT.ESCENARIO) ACT
...
AND DM.COD_ENTITY = ACT.COD_ENTITY AND DM.COD_SEGMENT = ACT.COD_SEGMENT AND DM.COD_ACCO_CENT = ACT.COD_ACCO_CENT AND DM.COD_EXPENSE = ACT.COD_EXPENSE
AND DM.EXP_FAM = ACT.EXP_FAM AND DM.EXP_TYPE = ACT.EXP_TYPE AND DM.COD_CURRENCY = ACT.COD_CURRENCY AND DM.ESCENARIO = ACT.ESCENARIO
...
LEFT JOIN (
    ...
    ACU.COD_ENTITY, ACU.COD_SEGMENT, ACU.COD_ACCO_CENT, ACU.COD_EXPENSE, ACU.EXP_FAM, ACU.EXP_TYPE, 
    CAST(SUM(ACU.NUM_CTOS) AS INT) AS NUM_CTOS, ACU.ESCENARIO 
    FROM MIS_APR_BUDGET ACU
    GROUP BY ACU.DATA_DATE, ACU.COD_CONT, ACU.COD_GL_GROUP, ACU.COD_BUSINESS_LINE, ACU.COD_PL_ACC, ACU.COD_BLCE_PROD, 
    ACU.COD_CURRENCY, ACU.IND_MENSUALIZA, ACU.COD_ENTITY, ACU.COD_SEGMENT, ACU.COD_ACCO_CENT, ACU.COD_EXPENSE, ACU.EXP_FAM, ACU.EXP_TYPE, ACU.ESCENARIO) ACU
...
AND DM.EXP_FAM = ACU.EXP_FAM AND DM.EXP_TYPE = ACU.EXP_TYPE AND DM.COD_CURRENCY = ACU.COD_CURRENCY AND DM.ESCENARIO = ACU.ESCENARIO 
...
LEFT JOIN (
    ...
    PRV.COD_ENTITY, PRV.COD_SEGMENT, PRV.COD_ACCO_CENT, PRV.COD_EXPENSE, PRV.EXP_FAM, PRV.EXP_TYPE, 
    CAST(SUM(PRV.NUM_CTOS) AS INT) AS NUM_CTOS, PRV.ESCENARIO 
...
AND DM.EXP_FAM = PRV.EXP_FAM AND DM.EXP_TYPE = PRV.EXP_TYPE
AND DM.COD_CURRENCY = PRV.COD_CURRENCY AND DM.ESCENARIO = PRV.ESCENARIO
AND DM.IND_MENSUALIZA = PRV.IND_MENSUALIZA AND
ADD_MONTHS(TO_TIMESTAMP(DM.DATA_DATE, 'yyyyMMdd'),-1) = TO_TIMESTAMP(PRV.DATA_DATE, 'yyyyMMdd') AND SUBSTR(DM.DATA_DATE, 5, 2) <> '01'
LEFT JOIN MIS_PAR_EXCH_RATE BLCE
ON BLCE.DATA_DATE = '${var:periodo}' AND BLCE.COD_CURRENCY = 'USD' AND BLCE.COD_CONT = 'PPTO_FOTO' 
AND IF(ISNULL(TRIM(BLCE.COD_ENTITY),'') = '' ,'1',DM.COD_ENTITY) = IF(ISNULL(TRIM(BLCE.COD_ENTITY),'') = '','1',BLCE.COD_ENTITY)
LEFT JOIN MIS_PAR_EXCH_RATE PYG
ON PYG.DATA_DATE = '${var:periodo}' AND PYG.COD_CURRENCY = 'USD' AND PYG.COD_CONT = 'PPTO_PROMEDIO' 
AND IF(ISNULL(TRIM(PYG.COD_ENTITY),'') = '' ,'1',DM.COD_ENTITY) = IF(ISNULL(TRIM(PYG.COD_ENTITY),'') = '','1',PYG.COD_ENTITY)
...
ACT.EOPBAL_CAP, ACT.AVGBAL_CAP, ACT.INI_AM, DM.COD_ENTITY, 
DM.COD_SEGMENT, DM.COD_ACCO_CENT, DM.COD_EXPENSE, DM.EXP_FAM, DM.EXP_TYPE, ACT.NUM_CTOS, DM.ESCENARIO, PYG.EXCH_RATE, BLCE.EXCH_RATE, ACT.COD_CURRENCY
