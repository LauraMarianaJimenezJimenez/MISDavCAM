...
INSERT INTO MIS_DM_BALANCE_RESULT_AUX
    (COD_CONT, COD_INFO_SOURCE, IDF_CTO, COD_CURRENCY, COD_GL_GROUP, DES_GL_GROUP, COD_ENTITY, COD_BLCE_PROD, 
    DES_BLCE_PROD, COD_PL_ACC, DES_PL_ACC,  COD_BUSINESS_LINE,  DES_BUSINESS_LINE, COD_VALUE, COD_SEGMENT, COD_OFFI, DES_OFFI,
    EOPBAL_CAP, AVGBAL_CAP,
    PL, INI_AM, CARD_NUMBER, DATE_DISB)
    PARTITION (DATA_DATE)
SELECT
    a.CONTENIDO, a.CRITERIO_DE_AJUSTE, a.CONTRATO AS IDF_CTO,  
    a.DIVISA AS COD_CURRENCY, b.COD_GL_GROUP, b.DES_GL_GROUP, b.COD_ENTITY,
    ...
    a.IMPORTE_ML AS EOPBAL_CAP, a.SALDO_MEDIO_ML AS AVGBAL_CAP,
    a.PL_ML AS PL, a.INI_AM_ML AS INI_AM, a.CARD_NUMBER, a.DATE_DISB, a.DATA_DATE
...
WHERE a.DATA_DATE='${var:periodo}' AND a.CRITERIO_DE_AJUSTE = 'ADJ_ADD'
;
...
COMPUTE INCREMENTAL STATS MIS_DM_BALANCE_RESULT_AUX partition (data_date = '${var:periodo}');
...
b.DATE_PRX_REV,b.EXP_DATE,b.FREQ_INT_PAY,b.COD_UNI_FREQ_INT_PAY,b.FRE_REV_INT,b.COD_UNI_FRE_REV_INT,b.AMRT_TRM,b.COD_UNI_AMRT_TRM,
    CASE WHEN dup.COD_SUBPRODUCT IS NOT NULL THEN b.INI_AM 
    WHEN m.COD_INFO_SOURCE LIKE '%_M' THEN 0 ELSE ISNULL(a.INI_AM,b.INI_AM) END AS INI_AM,b.CUO_AM,b.CREDIT_LIM_AM,b.PREDEF_RATE_IND,b.PREDEF_RATE,b.IND_CHANNEL,b.COD_SELLER,b.DES_SELLER,
    ...
    a.FTP,a.FTP_RESULT,a.IND_POOL,CASE WHEN b.COU_CAR_OFF IS NOT NULL THEN b.COU_CAR_OFF ELSE d.COU_CAR_OFF END AS COU_CAR_OFF, CASE WHEN b.COD_CONV IS NOT NULL THEN b.COD_CONV ELSE d.COD_CONV END AS COD_CONV, g.DES_CONV, CASE WHEN c.ID_ECO_GRO IS NOT NULL THEN c.ID_ECO_GRO ELSE e.ID_ECO_GRO END AS ID_ECO_GRO, CASE WHEN c.COU_ECO_GRO IS NOT NULL THEN c.COU_ECO_GRO ELSE e.COU_ECO_GRO END AS COU_ECO_GRO, f.DES_ECO_GRO, CASE WHEN c.IND_MUL_LAT IS NOT NULL THEN c.IND_MUL_LAT ELSE e.IND_MUL_LAT END AS IND_MUL_LAT, h.DES_MUL_LAT, b.COD_EXEC_CTO, b.COD_COVID_PORT, b.DATE_DISB, b.FIELD1_CTO, b.FIELD2_CTO, b.FIELD3_CTO, b.FIELD4_CTO, b.FIELD5_CTO, b.FIELD6_CTO, b.FIELD7_CTO, b.FIELD8_CTO, b.FIELD9_CTO, b.FIELD10_CTO, c.FIELD1_CLI, c.FIELD2_CLI, c.FIELD3_CLI, c.FIELD4_CLI, c.FIELD5_CLI, c.FIELD6_CLI, c.FIELD7_CLI, c.FIELD8_CLI, c.FIELD9_CLI, c.FIELD10_CLI, 
    a.COD_METHOD_FTP, i.COD_SECTOR_LOCAL, i.DES_SECTOR_LOCAL, i.COD_SECTOR_REG, i.DES_SECTOR_REG,  j.ID_ECO_GRO_REG,  k.DES_ECO_GRO_REG, 
    ISNULL(a.CARD_NUMBER,b.CARD_NUMBER) AS CARD_NUMBER, b.COD_PROG_CARD, b.DES_PROG_CARD, a.COD_ACT_TYPE, a.DATA_DATE
...
SELECT a.COD_CONT,a.IDF_CTO,a.COD_ACCO_CENT,a.COD_OFFI,a.COD_NAR,a.COD_EXPENSE,a.COD_BLCE_STATUS,a.COD_CURRENCY,a.COD_ENTITY,a.EXP_TYPE,a.EXP_FAMILY,a.COD_PRODUCT,a.COD_SUBPRODUCT,a.EOPBAL_TOT,a.EOPBAL_CAP,a.EOPBAL_INT,a.AVGBAL_TOT,a.AVGBAL_CAP,a.AVGBAL_INT,a.PL,a.COD_INFO_SOURCE,a.COD_TYP_FEE,a.IDF_CLI,a.TYP_DOC_ID,a.NUM_DOC_ID,a.NOM_NAME,a.NOM_LASTN,a.COD_RES_COUNT,a.COD_TYP_CLNT,a.COD_COND_CLNT,
...
SELECT 
    ...
    CAST(ROUND(X.FREQ_INT_PAY, 10) AS decimal(30, 10)) AS FREQ_INT_PAY, X.COD_UNI_FREQ_INT_PAY, CAST(ROUND(X.FRE_REV_INT, 10) AS decimal(30, 10)) AS FRE_REV_INT, 
    X.COD_UNI_FRE_REV_INT, CAST(ROUND(X.AMRT_TRM, 10) AS decimal(30, 10)) AS AMRT_TRM, X.COD_UNI_AMRT_TRM, CAST(ROUND(X.INI_AM, 10) AS decimal(30, 10)) AS INI_AM, 
CAST(ROUND(X.INI_AM_RPT, 10) AS decimal(30,10)) AS INI_AM_RPT,CAST(ROUND(X.INI_AM_USD, 10) AS decimal(30,10)) AS INI_AM_USD,CAST(ROUND(X.INI_AM_REG, 10) AS decimal(30,10)) AS INI_AM_REG,   
    CAST(ROUND(X.CUO_AM, 10) AS decimal(30, 10)) AS CUO_AM, CAST(ROUND(X.CREDIT_LIM_AM, 10) AS decimal(30, 10)) AS CREDIT_LIM_AM, X.PREDEF_RATE_IND, 
    CAST(ROUND(X.PREDEF_RATE, 10) AS decimal(30, 10)) AS PREDEF_RATE, X.IND_CHANNEL, X.COD_SELLER, X.DES_SELLER, X.COD_GL_GROUP, X.DES_GL_GROUP, X.ACCOUNT_CONCEPT, X.COD_PL_ACC, 
    X.DES_PL_ACC, X.COD_BLCE_PROD, X.DES_BLCE_PROD, X.COD_BUSINESS_LINE, X.DES_BUSINESS_LINE, X.IND_POOL, CAST(ROUND(SUM(X.EOPBAL_TOT), 10) AS decimal(30, 10)) AS EOPBAL_TOT, 
    CAST(ROUND(SUM(X.EOPBAL_CAP), 10) AS decimal(30, 10)) AS EOPBAL_CAP, CAST(ROUND(SUM(X.EOPBAL_INT), 10) AS decimal(30, 10)) AS EOPBAL_INT, 
    CAST(ROUND(SUM(X.AVGBAL_TOT), 10) AS decimal(30, 10)) AS AVGBAL_TOT, CAST(ROUND(SUM(X.AVGBAL_CAP), 10) AS decimal(30, 10)) AS AVGBAL_CAP, 
    CAST(ROUND(SUM(X.AVGBAL_INT), 10) AS decimal(30, 10)) AS AVGBAL_INT, CAST(ROUND(SUM(X.PL), 10) AS decimal(30, 10)) AS PL, 
    ...
    CAST(ROUND(SUM(X.PL_TOT_RPT), 10) AS decimal(30, 10)) AS PL_TOT_RPT, CAST(ROUND(SUM(X.PL_RPT), 10) AS decimal(30, 10)) AS PL_RPT, 
    ...
    X.DES_ACCO_CENT AS DES_OFFI,X.EXP_TYPE,X.EXP_FAMILY,X.COD_VALUE, X.COU_CAR_OFF, X.COD_CONV, X.DES_CONV, X.ID_ECO_GRO, X.COU_ECO_GRO, X.DES_ECO_GRO, X.IND_MUL_LAT, 
    X.DES_MUL_LAT, X.COD_EXEC_CTO, X.COD_COVID_PORT, X.DATE_DISB, X.FIELD1_CTO, X.FIELD2_CTO, X.FIELD3_CTO, X.FIELD4_CTO, X.FIELD5_CTO, X.FIELD6_CTO, X.FIELD7_CTO, X.FIELD8_CTO, 
    X.FIELD9_CTO, X.FIELD10_CTO, X.FIELD1_CLI, X.FIELD2_CLI, X.FIELD3_CLI, X.FIELD4_CLI, X.FIELD5_CLI, X.FIELD6_CLI, X.FIELD7_CLI, X.FIELD8_CLI, X.FIELD9_CLI, X.FIELD10_CLI,
...
X.COD_METHOD_FTP, 
X.COD_SECTOR_LOCAL, X.DES_SECTOR_LOCAL, X.COD_SECTOR_REG, X.DES_SECTOR_REG, X.ID_ECO_GRO_REG, X.DES_ECO_GRO_REG,
...
X.CARD_NUMBER, X.COD_PROG_CARD, X.DES_PROG_CARD, X.COD_ACT_TYPE, X.DATA_DATE 
FROM
    (SELECT 
...
A.INI_AM_RPT,
 ...
 A.CUO_AM, A.CREDIT_LIM_AM, 
        ...
        A.DES_ECO_GRO, A.IND_MUL_LAT, A.DES_MUL_LAT, A.COD_EXEC_CTO, A.COD_COVID_PORT, A.DATE_DISB, A.FIELD1_CTO, A.FIELD2_CTO, A.FIELD3_CTO, A.FIELD4_CTO, A.FIELD5_CTO, 
        A.FIELD6_CTO, A.FIELD7_CTO, A.FIELD8_CTO, A.FIELD9_CTO, A.FIELD10_CTO, A.FIELD1_CLI, A.FIELD2_CLI, A.FIELD3_CLI, A.FIELD4_CLI, A.FIELD5_CLI, A.FIELD6_CLI, A.FIELD7_CLI, 
        A.FIELD8_CLI, A.FIELD9_CLI, A.FIELD10_CLI, 
        CASE    
            WHEN A.COD_CURRENCY='USD' THEN IFNULL(SUM(A.PL_TOT),0)
            ELSE CAST(IFNULL(SUM(A.PL_TOT_RPT / e.EXCH_RATE),0) AS DECIMAL(30,10)) END AS PL_TOT_USD,
        CASE 
            WHEN A.COD_CURRENCY='USD' THEN IFNULL(SUM(A.PL_TOT), 0)
            ELSE CAST(IFNULL(SUM(A.PL_TOT_RPT / f.EXCH_RATE), 0) AS DECIMAL(30,10)) END AS PL_TOT_REG,
        CAST(IFNULL(SUM(A.EOPBAL_TOT_RPT / d.EXCH_RATE), 0) AS DECIMAL(30,10)) AS EOPBAL_TOT_USD,
        CAST(IFNULL(SUM(A.EOPBAL_TOT_RPT / c.EXCH_RATE), 0) AS DECIMAL(30,10)) AS EOPBAL_TOT_REG,        
        CAST(IFNULL(SUM(A.AVGBAL_TOT_RPT / d.EXCH_RATE), 0) AS DECIMAL(30,10)) AS AVGBAL_TOT_USD,
        CAST(IFNULL(SUM(A.AVGBAL_TOT_RPT / c.EXCH_RATE), 0) AS DECIMAL(30,10)) AS AVGBAL_TOT_REG, 
A.COD_METHOD_FTP, A.COD_SECTOR_LOCAL, A.DES_SECTOR_LOCAL, A.COD_SECTOR_REG, A.DES_SECTOR_REG, A.ID_ECO_GRO_REG, A.DES_ECO_GRO_REG,
        CASE    
            WHEN A.COD_CURRENCY='USD' THEN IFNULL(SUM(A.FTP_RESULT),0)
            ELSE CAST(IFNULL(SUM(A.FTP_RESULT_RPT / e.EXCH_RATE),0) AS DECIMAL(30,10)) END AS FTP_RESULT_USD,
        CASE 
            WHEN A.COD_CURRENCY='USD' THEN IFNULL(SUM(A.FTP_RESULT), 0)
            ELSE CAST(IFNULL(SUM(A.FTP_RESULT_RPT / f.EXCH_RATE), 0) AS DECIMAL(30,10)) END AS FTP_RESULT_REG,
A.CARD_NUMBER, A.COD_PROG_CARD, A.DES_PROG_CARD, A.COD_ACT_TYPE, A.DATA_DATE
    ...
    LEFT JOIN MIS_PAR_EXCH_RATE e   
        ON A.DATA_DATE=e.DATA_DATE AND e.COD_CONT = 'TC_PROMEDIO' AND e.COD_CURRENCY = 'USD'
        AND IF(ISNULL(TRIM(e.COD_ENTITY),'') = '' ,'1',A.COD_ENTITY) = IF(ISNULL(TRIM(e.COD_ENTITY),'') = '','1',e.COD_ENTITY)
    LEFT JOIN MIS_PAR_EXCH_RATE f
        ON A.DATA_DATE=f.DATA_DATE AND f.COD_CONT = 'REG_PROMEDIO' AND f.COD_CURRENCY = 'USD'
    ...
    A.EXP_TYPE, A.EXP_FAMILY, A.COD_VALUE, A.COU_CAR_OFF, A.COD_CONV, A.DES_CONV, A.ID_ECO_GRO, A.COU_ECO_GRO, A.DES_ECO_GRO, A.IND_MUL_LAT, A.DES_MUL_LAT, A.COD_EXEC_CTO, A.COD_COVID_PORT, A.DATE_DISB, A.FIELD1_CTO, A.FIELD2_CTO, A.FIELD3_CTO, A.FIELD4_CTO, A.FIELD5_CTO, A.FIELD6_CTO, A.FIELD7_CTO, A.FIELD8_CTO, A.FIELD9_CTO, A.FIELD10_CTO, A.FIELD1_CLI, A.FIELD2_CLI, A.FIELD3_CLI, A.FIELD4_CLI, A.FIELD5_CLI, A.FIELD6_CLI, A.FIELD7_CLI, A.FIELD8_CLI, A.FIELD9_CLI, A.FIELD10_CLI, A.COD_METHOD_FTP, A.COD_SECTOR_LOCAL, A.DES_SECTOR_LOCAL, A.COD_SECTOR_REG, A.DES_SECTOR_REG, A.ID_ECO_GRO_REG,  A.DES_ECO_GRO_REG, A.CARD_NUMBER, A.COD_PROG_CARD, A.DES_PROG_CARD, A.COD_ACT_TYPE
) X 
...
FROM 
    ...
    FROM
        ...
        WHERE DM.DATA_DATE='${var:periodo}' AND DM.IND_POOL IS NULL AND DM.COD_CONT NOT IN ('RCCL') 
        AND DM.COD_VALUE IN ('CAP','INT')) SALIDA_DM
    LEFT JOIN 
