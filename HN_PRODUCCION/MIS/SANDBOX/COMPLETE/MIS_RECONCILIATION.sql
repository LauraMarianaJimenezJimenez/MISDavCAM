...
ACCOUNT_CONCEPT STRING, EOPBAL_CAP DECIMAL(30,10), EOPBAL_INT DECIMAL(30,10), AVGBAL_CAP DECIMAL(30,10), AVGBAL_INT DECIMAL(30,10), PL DECIMAL(30,10));

INSERT INTO MIS_TMP_DWH_OPERACIONAL
SELECT TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, CASE WHEN STRLEFT(TMP.COD_GL_GROUP,1) IN ('5','6') THEN 'HNL' ELSE TMP.COD_CURRENCY END AS COD_CURRENCY, 
TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, 
    CASE WHEN TMP.COD_VALUE = 'INT' AND TMP.COD_CONT = 'PREST' THEN CAST(SUM(TMP.EOPBAL_CAP) * EXCF.EXCH_RATE AS decimal(30, 10)) ELSE CAST(SUM(TMP.EOPBAL_CAP) AS decimal(30, 10)) END AS EOPBAL_CAP, 
    CAST(SUM(TMP.EOPBAL_INT) AS decimal(30, 10)) AS EOPBAL_INT, CAST(SUM(TMP.AVGBAL_CAP) AS decimal(30, 10)) AS AVGBAL_CAP, 
    CAST(SUM(TMP.AVGBAL_INT) AS decimal(30, 10)) AS AVGBAL_INT, 
    CASE WHEN TMP.COD_CURRENCY = 'HNL' THEN CAST(SUM(TMP.PL) AS decimal(30, 10)) ELSE CAST(SUM(TMP.PL) * EXCP.EXCH_RATE AS decimal(30, 10)) END AS PL 
FROM MIS_DWH_ASSETS TMP
LEFT JOIN MIS_PAR_EXCH_RATE EXCF
ON TMP.DATA_DATE = EXCF.DATA_DATE AND TMP.COD_CURRENCY = EXCF.COD_CURRENCY AND EXCF.COD_CONT = 'TC_FOTO'
AND IF(ISNULL(TRIM(EXCF.COD_ENTITY),'') = '' ,'1',TMP.COD_ENTITY) = IF(ISNULL(TRIM(EXCF.COD_ENTITY),'') = '','1',EXCF.COD_ENTITY)
LEFT JOIN MIS_PAR_EXCH_RATE EXCP
ON TMP.DATA_DATE = EXCP.DATA_DATE AND TMP.COD_CURRENCY = EXCP.COD_CURRENCY AND EXCP.COD_CONT = 'TC_PROMEDIO'
AND IF(ISNULL(TRIM(EXCP.COD_ENTITY),'') = '' ,'1',TMP.COD_ENTITY) = IF(ISNULL(TRIM(EXCP.COD_ENTITY),'') = '','1',EXCP.COD_ENTITY)
WHERE TMP.DATA_DATE='${var:periodo}'
GROUP BY TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, COD_VALUE, 
EXCF.EXCH_RATE, EXCP.EXCH_RATE
UNION ALL
SELECT TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, CASE WHEN STRLEFT(TMP.COD_GL_GROUP,1) IN ('5','6') THEN 'HNL' ELSE TMP.COD_CURRENCY END AS COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT,
    CAST(SUM(TMP.EOPBAL_CAP) AS decimal(30, 10)) AS EOPBAL_CAP, CAST(SUM(TMP.EOPBAL_INT) AS decimal(30, 10)) AS EOPBAL_INT, CAST(SUM(TMP.AVGBAL_CAP) AS decimal(30, 10)) AS AVGBAL_CAP, CAST(SUM(TMP.AVGBAL_INT) AS decimal(30, 10)) AS AVGBAL_INT, CAST(SUM(TMP.PL) AS decimal(30, 10)) AS PL 
...
SELECT TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, CASE WHEN STRLEFT(TMP.COD_GL_GROUP,1) IN ('5','6') THEN 'HNL' ELSE TMP.COD_CURRENCY END AS COD_CURRENCY, 
TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, 
    CAST(SUM(TMP.EOPBAL_CAP) AS decimal(30, 10)) AS EOPBAL_CAP, CAST(SUM(TMP.EOPBAL_INT) AS decimal(30, 10)) AS EOPBAL_INT, 
    CAST(SUM(TMP.AVGBAL_CAP) AS decimal(30, 10)) AS AVGBAL_CAP, CAST(SUM(TMP.AVGBAL_INT) AS decimal(30, 10)) AS AVGBAL_INT, 
    CASE WHEN TMP.COD_CURRENCY = 'HNL' THEN CAST(SUM(TMP.PL) AS decimal(30, 10)) ELSE CAST(SUM(TMP.PL) * EXCP.EXCH_RATE AS decimal(30, 10)) END AS PL 
...
AND IF(ISNULL(TRIM(EXCP.COD_ENTITY),'') = '' ,'1',TMP.COD_ENTITY) = IF(ISNULL(TRIM(EXCP.COD_ENTITY),'') = '','1',EXCP.COD_ENTITY)
WHERE TMP.DATA_DATE='${var:periodo}'
GROUP BY TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, EXCF.EXCH_RATE, EXCP.EXCH_RATE
UNION ALL
SELECT TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, CASE WHEN STRLEFT(TMP.COD_GL_GROUP,1) IN ('5','6') THEN 'HNL' ELSE TMP.COD_CURRENCY END AS COD_CURRENCY, 
TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, 
    CAST(SUM(TMP.EOPBAL_CAP) AS decimal(30, 10)) AS EOPBAL_CAP, CAST(SUM(TMP.EOPBAL_INT) AS decimal(30, 10)) AS EOPBAL_INT, CAST(SUM(TMP.AVGBAL_CAP) AS decimal(30, 10)) AS AVGBAL_CAP, 
    CAST(SUM(TMP.AVGBAL_INT) AS decimal(30, 10)) AS AVGBAL_INT, CASE WHEN TMP.COD_CURRENCY = 'HNL' THEN CAST(SUM(TMP.PL) AS decimal(30, 10)) ELSE CAST(SUM(TMP.PL) * EXCP.EXCH_RATE AS decimal(30, 10)) END AS PL
...
UNION ALL
SELECT TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, CASE WHEN STRLEFT(TMP.COD_GL_GROUP,1) IN ('5','6') THEN 'HNL' ELSE TMP.COD_CURRENCY END AS COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, 
    CAST(SUM(TMP.EOPBAL_CAP) AS decimal(30, 10)) AS EOPBAL_CAP, CAST(SUM(TMP.EOPBAL_INT) AS decimal(30, 10)) AS EOPBAL_INT, CAST(SUM(TMP.AVGBAL_CAP) AS decimal(30, 10)) AS AVGBAL_CAP, CAST(SUM(TMP.AVGBAL_INT) AS decimal(30, 10)) AS AVGBAL_INT, CASE WHEN TMP.COD_CURRENCY = 'HNL' THEN CAST(SUM(TMP.PL) AS decimal(30, 10)) ELSE CAST(SUM(TMP.PL) * EXCP.EXCH_RATE AS decimal(30, 10)) END AS PL 
...
SELECT TMP.DATA_DATE, TMP.COD_CONT, NULL AS COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, 
    CAST(SUM(TMP.EOPBAL_CAP) AS decimal(30, 10)) AS EOPBAL_CAP, CAST(SUM(TMP.EOPBAL_INT) AS decimal(30, 10)) AS EOPBAL_INT, NULL AS AVGBAL_CAP, NULL AS AVGBAL_INT, CAST(SUM(TMP.PL) AS decimal(30, 10)) AS PL 
...
--COMPUTE STATS MIS_TMP_DWH_OPERACIONAL;
COMPUTE INCREMENTAL STATS MIS_DWH_ACCOUNTING partition (data_date = '${var:periodo}');

--- Generación de conciliación a partir de las tablas apropiadas, verificando si la información es netamente contable, operacional o proviene de ambas fuentes para lo diferente a gastos ------------
...
--- Selecciona las variables del flujo contable u operacional, según corresponda ---         
    ...
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN CAST((ISNULL(CONT.TOT_EOPBAL_CAP,0) - ISNULL(OPER.TOT_EOPBAL_CAP,0)) AS decimal(15, 2)) 
    ...
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN CAST((ISNULL(CONT.TOT_EOPBAL_INT,0) - ISNULL(OPER.TOT_EOPBAL_INT,0)) AS decimal(15, 2)) 
    ...
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN 0 /*CAST((ISNULL(CONT.TOT_AVGBAL_CAP,0) - ISNULL(OPER.TOT_AVGBAL_CAP,0)) AS decimal(15, 2)) */
         WHEN CONT.DATA_DATE IS NOT NULL THEN CAST((CONT.TOT_AVGBAL_CAP) AS decimal(15, 2))
         WHEN OPER.DATA_DATE IS NOT NULL THEN CAST((-1 * OPER.TOT_AVGBAL_CAP) AS decimal(15, 2)) END AS AVGBAL_CAP, 
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN 0 /*CAST((ISNULL(CONT.TOT_AVGBAL_INT,0) - ISNULL(OPER.TOT_AVGBAL_INT,0)) AS decimal(15, 2)) */
         WHEN CONT.DATA_DATE IS NOT NULL THEN CAST((CONT.TOT_AVGBAL_INT) AS decimal(15, 2))
         WHEN OPER.DATA_DATE IS NOT NULL THEN CAST((-1 * OPER.TOT_AVGBAL_INT) AS decimal(15, 2)) END AS AVGBAL_INT,
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN CAST((ISNULL(CONT.TOT_PL,0) - ISNULL(OPER.TOT_PL,0)) AS decimal(15, 2)) 
...
FROM (
    SELECT TMP.DATA_DATE, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP,
        TMP.ACCOUNT_CONCEPT, SUM(TMP.EOPBAL_CAP) AS TOT_EOPBAL_CAP,SUM(TMP.EOPBAL_INT) AS TOT_EOPBAL_INT, SUM(TMP.AVGBAL_CAP) AS TOT_AVGBAL_CAP, SUM(TMP.AVGBAL_INT) AS TOT_AVGBAL_INT, SUM(TMP.PL) AS TOT_PL 
    FROM MIS_TMP_DWH_OPERACIONAL TMP
    WHERE TMP.DATA_DATE='${var:periodo}' AND TMP.COD_GL_GROUP IN (SELECT DISTINCT COD_GL_GROUP FROM MIS_PAR_REL_CAF_OPER)
    GROUP BY TMP.DATA_DATE, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP,
        TMP.ACCOUNT_CONCEPT) OPER 
FULL JOIN (
    SELECT ACC.DATA_DATE, ACC.COD_CURRENCY, ACC.COD_ENTITY, ACC.COD_GL_GROUP, ACC.DES_GL_GROUP, 
        ACC.ACCOUNT_CONCEPT,
        SUM(ACC.EOPBAL_CAP) AS TOT_EOPBAL_CAP, SUM(ACC.EOPBAL_INT) AS TOT_EOPBAL_INT, SUM(ACC.AVGBAL_CAP) AS TOT_AVGBAL_CAP, SUM(ACC.AVGBAL_INT) AS TOT_AVGBAL_INT, SUM(ACC.PL) AS TOT_PL
    FROM MIS_DWH_ACCOUNTING ACC
    WHERE ACC.DATA_DATE='${var:periodo}' AND ACC.COD_GL_GROUP IN (SELECT DISTINCT COD_GL_GROUP FROM MIS_PAR_REL_CAF_OPER)
    GROUP BY ACC.DATA_DATE, ACC.COD_CURRENCY, ACC.COD_ENTITY, ACC.COD_GL_GROUP, ACC.DES_GL_GROUP,
        ACC.ACCOUNT_CONCEPT) CONT 
ON OPER.DATA_DATE=CONT.DATA_DATE AND OPER.COD_CURRENCY=CONT.COD_CURRENCY AND 
   OPER.COD_ENTITY=CONT.COD_ENTITY AND OPER.COD_GL_GROUP=CONT.COD_GL_GROUP  
LEFT JOIN (SELECT DISTINCT COD_GL_GROUP FROM MIS_PAR_REL_CAF_OPER) COD_OPER
ON CONT.COD_GL_GROUP = COD_OPER.COD_GL_GROUP;


--- Generación de conciliación a partir de las tablas apropiadas, verificando si la información es netamente contable, operacional o proviene de ambas fuentes para gastos------------
INSERT INTO MIS_DWH_RECONCILIATION_AUX
SELECT 
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN CONT.DATA_DATE
         WHEN CONT.DATA_DATE IS NOT NULL THEN CONT.DATA_DATE 
...
FROM (
    SELECT TMP.DATA_DATE, '0' AS COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP,
        TMP.ACCOUNT_CONCEPT, SUM(TMP.EOPBAL_CAP) AS TOT_EOPBAL_CAP,SUM(TMP.EOPBAL_INT) AS TOT_EOPBAL_INT, SUM(TMP.AVGBAL_CAP) AS TOT_AVGBAL_CAP, SUM(TMP.AVGBAL_INT) AS TOT_AVGBAL_INT, SUM(TMP.PL) AS TOT_PL 
    FROM MIS_TMP_DWH_OPERACIONAL TMP
    WHERE TMP.DATA_DATE='${var:periodo}' AND TMP.COD_GL_GROUP NOT IN (SELECT DISTINCT COD_GL_GROUP FROM MIS_PAR_REL_CAF_OPER)
    GROUP BY TMP.DATA_DATE, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP,
        TMP.ACCOUNT_CONCEPT) OPER 
FULL JOIN (
    SELECT ACC.DATA_DATE, ACC.COD_ACCO_CENT, ACC.COD_CURRENCY, ACC.COD_ENTITY, ACC.COD_GL_GROUP, ACC.DES_GL_GROUP, 
        ...
        SUM(ACC.EOPBAL_CAP) AS TOT_EOPBAL_CAP, SUM(ACC.EOPBAL_INT) AS TOT_EOPBAL_INT, SUM(ACC.AVGBAL_CAP) AS TOT_AVGBAL_CAP, SUM(ACC.AVGBAL_INT) AS TOT_AVGBAL_INT, SUM(ACC.PL) AS TOT_PL
    FROM MIS_DWH_ACCOUNTING ACC
    WHERE ACC.DATA_DATE='${var:periodo}' AND ACC.COD_GL_GROUP NOT IN (SELECT DISTINCT COD_GL_GROUP FROM MIS_PAR_REL_CAF_OPER)
...
--COMPUTE STATS MIS_DWH_RECONCILIATION_AUX;
...
--COMPUTE STATS MIS_TMP_STG_RECONC_1;
...
--COMPUTE STATS MIS_TMP_STG_RECONC_2;
...
--COMPUTE STATS MIS_TMP_STG_RECONC_3;
...
SELECT DWH.COD_ACCO_CENT, DWH.COD_OFFI, DWH.COD_NAR, '0' AS COD_BLCE_STATUS, DWH.COD_CURRENCY, DWH.COD_ENTITY,
    ...
    DWH.COD_BLCE_PROD, DWH.DES_BLCE_PROD, DWH.COD_BUSINESS_LINE, DWH.DES_BUSINESS_LINE, NULL, NULL, NULL, NULL, NULL, 
    DWH.EXP_TYPE, DWH.EXP_FAMILY, DWH.DATA_DATE, DWH.COD_CONT 
FROM MIS_TMP_STG_RECONC_4 DWH 
WHERE DWH.COD_BUSINESS_LINE IS NOT NULL;

--- código para saldo medios de partidas conciliatorias y cuenta netamente contables ---
...
INSERT INTO MIS_TMP_LOAD_AVGBAL_CONC
    SELECT a.COD_GL_GROUP, COD_BLCE_STATUS, a.COD_ENTITY, COD_CURRENCY, a.COD_ACCO_CENT, 'A' AS COD_INFO_SOURCE, COD_BLCE_PROD, DES_BLCE_PROD,
...
LEFT JOIN (SELECT COD_GL_GROUP, COD_ACCO_CENT, COD_ENTITY, CAST((SUM(EOPBAL_CAP)/(DATEDIFF(to_timestamp('${var:periodo}','yyyyMMdd'),TRUNC(to_timestamp('${var:periodo}','yyyyMMdd'),'MM'))+1)) AS DECIMAL(30,10))AS AVGBAL_CAP,
 CAST((SUM(EOPBAL_INT)/(DATEDIFF(to_timestamp('${var:periodo}','yyyyMMdd'),TRUNC(to_timestamp('${var:periodo}','yyyyMMdd'),'MM'))+1)) AS DECIMAL(30,10))AS AVGBAL_INT
    ...
    GROUP BY COD_GL_GROUP, COD_ACCO_CENT, COD_ENTITY) b
ON a.COD_GL_GROUP = b.COD_GL_GROUP AND a.COD_ACCO_CENT = b.COD_ACCO_CENT AND a.COD_ENTITY = b.COD_ENTITY
WHERE DATA_DATE =  '${var:periodo}'
    AND COD_PL_ACC = 'NO_PYG'
    AND COD_CONT IN ('RCCL','RCBA','RCIN','RCLI','CTBP','RCRP')
    GROUP BY a.COD_GL_GROUP, COD_BLCE_STATUS, a.COD_ENTITY, COD_CURRENCY, a.COD_ACCO_CENT, COD_BLCE_PROD, DES_BLCE_PROD,
...
INSERT INTO MIS_DWH_RECONCILIATION PARTITION (DATA_DATE, COD_CONT)
SELECT DWH.COD_ACCO_CENT, DWH.COD_OFFI, DWH.COD_NAR, DWH.COD_BLCE_STATUS, DWH.COD_CURRENCY, DWH.COD_ENTITY,
    DWH.EOPBAL_CAP, DWH.EOPBAL_INT, b.AVGBAL_CAP, b.AVGBAL_INT, DWH.PL, DWH.COD_INFO_SOURCE, 
