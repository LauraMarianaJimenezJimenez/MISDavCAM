...
SELECT DWH.COD_CONT, DWH.IDF_CTO, DWH.COD_GL, DWH.DES_GL, DWH.COD_ACCO_CENT, DWH.COD_OFFI, DWH.COD_BLCE_STATUS, DWH.COD_VALUE, DWH.COD_CURRENCY, DWH.COD_ENTITY, DWH.COD_PRODUCT, DWH.COD_SUBPRODUCT, 
    DWH.COD_ACT_TYPE, DWH.EOPBAL_CAP, DWH.EOPBAL_INT, DWH.AVGBAL_CAP, DWH.AVGBAL_INT, DWH.PL, DWH.COD_INFO_SOURCE, DWH.COD_GL_GROUP, 
    DWH.COD_PL_ACC, DWH.DES_PL_ACC, DWH.COD_BLCE_PROD, DWH.DES_BLCE_PROD, DWH.COD_BUSINESS_LINE, DWH.DES_BUSINESS_LINE, DWH.DES_GL_GROUP, DWH.ACCOUNT_CONCEPT,
    ...
    CASE WHEN DT.CARD_NUMBER IS NOT NULL AND DWH.EOPBAL_CAP < 0 THEN 'NEG' WHEN TTI.IDF_CTO IS NOT NULL THEN 'SP' ELSE NULL END AS 
    COD_METHOD, CLI.COD_SEGMENT, DT.COD_AMRT_MET
FROM 
    (SELECT TMP.*
...
SELECT DWH.COD_CONT, DWH.IDF_CTO, DWH.COD_GL, DWH.DES_GL, DWH.COD_ACCO_CENT, DWH.COD_OFFI, DWH.COD_BLCE_STATUS, DWH.COD_VALUE, DWH.COD_CURRENCY, DWH.COD_ENTITY, DWH.COD_PRODUCT, DWH.COD_SUBPRODUCT, 
    DWH.COD_ACT_TYPE, CAST(DWH.EOPBAL_CAP AS decimal(30, 10)), CAST(DWH.EOPBAL_INT AS decimal(30, 10)), CAST(DWH.AVGBAL_CAP AS decimal(30, 10)), CAST(DWH.AVGBAL_INT AS decimal(30, 10)), CAST(DWH.PL AS decimal(30, 10)), DWH.COD_INFO_SOURCE, DWH.COD_GL_GROUP, 
    DWH.COD_PL_ACC, DWH.DES_PL_ACC, DWH.COD_BLCE_PROD, DWH.DES_BLCE_PROD, DWH.COD_BUSINESS_LINE, DWH.DES_BUSINESS_LINE, DWH.DES_GL_GROUP, DWH.ACCOUNT_CONCEPT, 
...
FROM 
    ...
    SUM(TMP.PL) AS PL, ANT.COD_GL_GROUP, TMP.COD_PL_ACC, TMP.DES_PL_ACC, ANT.COD_BLCE_PROD, ANT.DES_BLCE_PROD,
    TMP.COD_BUSINESS_LINE, TMP.DES_BUSINESS_LINE, ANT.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT, TMP.COD_METHOD1, TMP.FTP1, TMP.COD_METHOD2, TMP.FTP2, TMP.FTP_RESULT, TMP.IND_POOL, TMP.DATA_DATE, TMP.COD_INFO_SOURCE
    FROM MIS_DWH_${var:tabla} TMP 
    LEFT JOIN (
        SELECT IDF_CTO, CON AS CONTEO
        FROM (SELECT IDF_CTO, COUNT(*) AS CON
        FROM MIS_DWH_${var:tabla} LIA
        WHERE LIA.DATA_DATE = '${var:periodo}' AND LIA.COD_VALUE IN ('CAP','INT') AND LIA.FTP1 IS NULL AND STRRIGHT(LIA.cod_info_source,2) = '_M'
        GROUP BY IDF_CTO) A
        ) CONT
    ON TMP.IDF_CTO = CONT.IDF_CTO
    ...
    WHERE TMP.DATA_DATE = '${var:periodo}' AND TMP.COD_VALUE IN ('CAP','INT') AND TMP.FTP1 IS NULL
...
SELECT a.*,
    ...
    CASE WHEN a.FTP IS NOT NULL THEN 'N' --No se rechazan contratos con TTI especial
        ...
        WHEN ISNULL(b.COD_BLCE_PROD,c.COD_BLCE_PROD) IS NULL THEN 'E' --Se excluye ya que al no estar parametrizado ni se debe rechazar ni se le debe calcular TTI(revisar)
        ...
        WHEN ISNULL(b.METHOD_TTI,c.METHOD_TTI) = 'MA' AND (ISNULL(b.AVG_PERIOD,c.AVG_PERIOD) IS NULL OR ISNULL(b.AVG_PERIOD,c.AVG_PERIOD) <= 0) THEN 'Y' 
        WHEN ISNULL(b.METHOD_TTI,c.METHOD_TTI) = 'EX' AND (ISNULL(b.TERM_FACTOR,c.TERM_FACTOR) IS NULL OR 
        ISNULL(b.TERM_FACTOR,c.TERM_FACTOR) <= 0 OR ISNULL(b.TERM_FACTOR,c.TERM_FACTOR) >= 1) THEN 'Y' 
        WHEN ISNULL(b.METHOD_TTI,c.METHOD_TTI) IN ('ST','EX') AND (a.DATE_ORIGIN IS NULL OR a.EXP_DATE IS NULL) THEN 'Y' 
        WHEN ISNULL(b.METHOD_TTI,c.METHOD_TTI) IN ('ST','EX') AND a.COD_RATE_TYPE = 'V' AND (a.DATE_LAST_REV IS NULL OR (a.EXP_DATE IS NULL AND a.DATE_PRX_REV IS NULL)) THEN 'Y' 
...
SELECT a.DATA_DATE, a.IDF_CTO, a.COD_BLCE_PROD, a.COD_BUSINESS_LINE, a.METHOD_TTI, a.COD_CURVE, a.COD_CURRENCY_TTI, a.COD_BCA_INT, 
    ...
    IF(a.COD_VALUE='INT', a.EOPBAL_INT, a.EOPBAL_CAP) AS EOPBAL, 
    a.COD_CONT 
...
SELECT ISNULL(PARAM_ST.METHOD_TTI,PARAM_ST2.METHOD_TTI) AS METHOD_TTI, ISNULL(PARAM_ST.COD_CURVE,PARAM_ST2.COD_CURVE) AS COD_CURVE, ISNULL(PARAM_ST.TERM_FACTOR,PARAM_ST2.TERM_FACTOR) AS TERM_FACTOR, ISNULL(PARAM_ST.IND_SPREAD,PARAM_ST2.IND_SPREAD) AS IND_SPREAD, ISNULL(PARAM_ST.IND_VOLATILITY,PARAM_ST2.IND_VOLATILITY) AS IND_VOL, 
ISNULL(PARAM_ST.PERC_VOLATILITY,PARAM_ST2.PERC_VOLATILITY) AS PERC_VOLATILITY, SALIDA_DWH.*,  
    CASE
        WHEN ISNULL(PARAM_ST.METHOD_TTI,PARAM_ST2.METHOD_TTI) = 'EX'
        THEN ROUND(DATEDIFF(TO_TIMESTAMP(SALIDA_DWH.FINISH_DATE,'yyyyMMdd'), TO_TIMESTAMP(SALIDA_DWH.START_DATE,'yyyyMMdd')) * ISNULL(PARAM_ST.TERM_FACTOR,PARAM_ST2.TERM_FACTOR),0)
...
FROM
    ...
    CASE WHEN DWH.COD_RATE_TYPE = 'A' THEN DWH.DATE_PRX_REV 
    ...
    WHERE DWH.FTP IS NULL
        AND ((DWH.COD_RATE_TYPE = 'A' AND DWH.DATE_LAST_REV = DWH.DATA_DATE)
...
SELECT ST.DATA_DATE, ST.COD_CONT, ST.IDF_CTO, ST.COD_GL, ST.DES_GL, ST.COD_ACCO_CENT, ST.COD_OFFI, ST.COD_BLCE_STATUS, ST.COD_VALUE, ST.COD_CURRENCY,
    ...
    CAST(CASE
        WHEN ST.COD_CONT = 'PLZ' AND ST.EXP_DATE < ST.DATA_DATE THEN 0
...
ON TO_TIMESTAMP(DWH_PREV.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP(ST.DATA_DATE, 'yyyyMMdd'), 1) AND DWH_PREV.COD_VALUE = ST.COD_VALUE AND 
    CASE WHEN DWH_PREV.COD_SUBPRODUCT LIKE '%INT%' AND STRRIGHT(DWH_PREV.IDF_CTO,1) <> 'I' THEN CONCAT(DWH_PREV.IDF_CTO,'I') ELSE DWH_PREV.IDF_CTO END = ST.IDF_CTO AND DWH_PREV.COD_ENTITY=ST.COD_ENTITY AND --AND DWH_PREV.COD_PRODUCT = ST.COD_PRODUCT AND DWH_PREV.COD_SUBPRODUCT = ST.COD_SUBPRODUCT 
...
ON TO_TIMESTAMP(DT_PREV.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP(ST.DATA_DATE, 'yyyyMMdd'), 1) AND 
    CASE WHEN DT_PREV.COD_SUBPRODUCT LIKE '%INT%' AND STRRIGHT(DT_PREV.IDF_CTO,1) <> 'I' THEN CONCAT(DT_PREV.IDF_CTO,'I') ELSE DT_PREV.IDF_CTO END = ST.IDF_CTO AND DT_PREV.COD_ENTITY=ST.COD_ENTITY AND DT_PREV.COD_CURRENCY=ST.COD_CURRENCY;
...
SELECT a.DATA_DATE, a.COD_CONT, a.IDF_CTO, a.COD_GL, a.DES_GL, a.COD_ACCO_CENT, a.COD_OFFI, a.COD_BLCE_STATUS, a.COD_VALUE, a.COD_CURRENCY, a.COD_ENTITY, 
    ...
    a.COD_BUSINESS_LINE, a.DES_BUSINESS_LINE, CAST(CASE WHEN a.COD_CONT = 'PLZ' AND a.EXP_DATE < a.DATA_DATE THEN 0 ELSE b.TTI END AS DECIMAL(30,10)) AS FTP, a.FTP_RESULT, 'N' AS IND_POOL, 
...
SELECT LPE.DATA_DATE, LPE.COD_CONT, LPE.IDF_CTO, LPE.COD_GL, LPE.DES_GL, LPE.COD_ACCO_CENT, LPE.COD_OFFI, LPE.COD_BLCE_STATUS, LPE.COD_VALUE, LPE.COD_CURRENCY,
    ...
    CAST(CASE 
        WHEN LPE.COD_CONT = 'PLZ' AND LPE.EXP_DATE < LPE.DATA_DATE THEN 0
...
ON TO_TIMESTAMP(DT_PREV.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP(LPE.DATA_DATE, 'yyyyMMdd'), 1) AND 
    CASE WHEN DT_PREV.COD_SUBPRODUCT LIKE '%INT%' AND STRRIGHT(DT_PREV.IDF_CTO,1) <> 'I' THEN CONCAT(DT_PREV.IDF_CTO,'I') ELSE DT_PREV.IDF_CTO END = LPE.IDF_CTO AND DT_PREV.COD_ENTITY = LPE.COD_ENTITY AND DT_PREV.COD_CURRENCY = LPE.COD_CURRENCY
LEFT JOIN (
...
LEFT JOIN (
     ...
     TO_TIMESTAMP(TMP.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'), 1) AND FTP_RESULT <> 0 AND IND_POOL = 'N') 
     DM --Propagar valor del día anterior
ON TO_TIMESTAMP(DM.DATA_DATE, 'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP(DWH.DATA_DATE,'yyyyMMdd'), 1)
    AND DM.COD_CONT=DWH.COD_CONT AND 
    CASE WHEN DM.COD_SUBPRODUCT LIKE '%INT%' AND STRRIGHT(DM.IDF_CTO,1) <> 'I' THEN CONCAT(DM.IDF_CTO,'I') ELSE DM.IDF_CTO END = DWH.IDF_CTO 
    AND DM.COD_CURRENCY=DWH.COD_CURRENCY AND DM.COD_ENTITY=DWH.COD_ENTITY AND DM.COD_VALUE = DWH.COD_VALUE 
    AND DM.COD_INFO_SOURCE=DWH.COD_INFO_SOURCE /*AND DM.COD_ACCO_CENT=DWH.COD_ACCO_CENT AND DM.COD_BLCE_STATUS=DWH.COD_BLCE_STATUS
    AND DM.COD_PRODUCT=DWH.COD_PRODUCT AND DM.COD_SUBPRODUCT=DWH.COD_SUBPRODUCT*/
;
...
LEFT JOIN (
    ...
    FROM MIS_DWH_${var:tabla} TMP
    WHERE TO_TIMESTAMP(TMP.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'), 1)) DM 
ON TO_TIMESTAMP(DM.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP(TTP.DATA_DATE, 'yyyyMMdd'), 1)
    AND DM.COD_CONT = TTP.COD_CONT AND DM.IDF_CTO = TTP.IDF_CTO AND DM.COD_CURRENCY = TTP.COD_CURRENCY 
    AND DM.COD_ENTITY = TTP.COD_ENTITY 
    AND DM.COD_VALUE = TTP.COD_VALUE AND DM.IND_POOL = TTP.IND_POOL AND DM.COD_INFO_SOURCE = TTP.COD_INFO_SOURCE
    /*AND DM.COD_ACCO_CENT=TTP.COD_ACCO_CENT AND DM.COD_BLCE_STATUS=TTP.COD_BLCE_STATUS
    AND DM.COD_PRODUCT=TTP.COD_PRODUCT AND DM.COD_SUBPRODUCT=TTP.COD_SUBPRODUCT*/
;
