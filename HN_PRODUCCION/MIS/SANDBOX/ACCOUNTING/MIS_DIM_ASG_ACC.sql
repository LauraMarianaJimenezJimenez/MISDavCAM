...
HAVING SUM(DWH.EOPBAL_CAP) <> 0 OR SUM(DWH.PL) <> 0; --Inicamente se guardan rechazos con valores diferente de cero
...
SELECT DWH.COD_CONT, DWH.COD_GL, DWH.DES_GL, DWH.COD_ACCO_CENT, DWH.COD_OFFI, DWH.COD_NAR, DWH.COD_BLCE_STATUS, DWH.COD_CURRENCY, DWH.COD_ENTITY, DWH.EOPBAL_CAP, DWH.EOPBAL_INT, 
    ...
    CASE WHEN DWH.COD_CURRENCY="HNL" THEN CAST(DWH.EOPBAL_CAP AS decimal(30, 10))
        ELSE CAST(DWH.EOPBAL_CAP * RTF.EXCH_RATE AS decimal(30, 10))
        END AS EOPBAL_CAP_RPT, 
    CASE WHEN DWH.COD_CURRENCY="HNL" THEN CAST(DWH.PL AS decimal(30, 10))
        ELSE CAST(DWH.PL * RTP.EXCH_RATE AS decimal(30, 10)) 
        END AS PL_RPT, 
...
LEFT JOIN MIS_PAR_EXCH_RATE RTF
ON DWH.DATA_DATE = RTF.DATA_DATE AND RTF.COD_CONT= 'TC_FOTO' AND DWH.COD_CURRENCY = RTF.COD_CURRENCY
AND IF(ISNULL(TRIM(RTF.COD_ENTITY),'') = '' ,'1',DWH.COD_ENTITY) = IF(ISNULL(TRIM(RTF.COD_ENTITY),'') = '','1',RTF.COD_ENTITY)
LEFT JOIN MIS_PAR_EXCH_RATE RTP
ON DWH.DATA_DATE = RTP.DATA_DATE AND RTP.COD_CONT= 'TC_PROMEDIO' AND DWH.COD_CURRENCY = RTP.COD_CURRENCY
AND IF(ISNULL(TRIM(RTP.COD_ENTITY),'') = '' ,'1',DWH.COD_ENTITY) = IF(ISNULL(TRIM(RTP.COD_ENTITY),'') = '','1',RTP.COD_ENTITY)
;
