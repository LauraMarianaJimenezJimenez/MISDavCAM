...
GROUP BY TMP.DATA_DATE, TMP.COD_CONT, TMP.COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP, TMP.ACCOUNT_CONCEPT
UNION ALL
SELECT TMP.DATA_DATE, TMP.CONTENIDO, TMP.CENTRO_CONTABLE, TMP.DIVISA, TMP.ENTIDAD, TMP.AGRUPADOR_CONTABLE, c.DES_GL_GROUP, c.ACCOUNT_CONCEPT, 
    CAST(SUM(TMP.IMPORTE_ML) AS decimal(30, 10)) AS EOPBAL_CAP, 0 AS EOPBAL_INT, CAST(SUM(TMP.PL_ML) AS decimal(30, 10)) AS PL 
FROM MIS_DWH_MANUAL_ADJ_OPER TMP
LEFT JOIN MIS_PAR_CAT_CAF c 
ON TMP.AGRUPADOR_CONTABLE = c.COD_GL_GROUP AND TMP.ENTIDAD = c.COD_ENTITY AND TMP.DIVISA=c.COD_CURRENCY
WHERE TMP.DATA_DATE='${var:periodo}'
...
--- Selecciona las variables del flujo contable u operacional, según corresponda ---         
    ...
    0 AS AVGBAL_CAP, 0 AS AVGBAL_INT,
...
FROM (
    SELECT TMP.DATA_DATE, '0' AS COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP,
        TMP.ACCOUNT_CONCEPT, SUM(TMP.EOPBAL_CAP) AS TOT_EOPBAL_CAP,SUM(TMP.EOPBAL_INT) AS TOT_EOPBAL_INT, SUM(TMP.PL) AS TOT_PL 
    FROM MIS_TMP_DWH_OPERACIONAL_M TMP
    WHERE TMP.DATA_DATE='${var:periodo}'
...
FULL JOIN (
    SELECT ACC.DATA_DATE, ACC.COD_ACCO_CENT, ACC.COD_CURRENCY, ACC.COD_ENTITY, ACC.COD_GL_GROUP, ACC.DES_GL_GROUP, 
        ACC.ACCOUNT_CONCEPT,
        SUM(ACC.EOPBAL_CAP) AS TOT_EOPBAL_CAP, SUM(ACC.EOPBAL_INT) AS TOT_EOPBAL_INT, SUM(ACC.PL) AS TOT_PL
...
INSERT INTO MIS_REJECTIONS_DIMENSIONS_M (DATA_DATE, DIM_REJ, COD_ENTITY, COD_CURRENCY, COD_GL, COD_GL_GROUP, COD_ACT_TYPE, COD_PRODUCT, COD_SUBPRODUCT, COD_RATE_TYPE, COD_BLCE_STATUS, COD_VALUE, 
...
SELECT DWH.DATA_DATE, 'CUENTA DE GESTION' AS DIM_REJ, DWH.COD_ENTITY, DWH.COD_CURRENCY, 'N/A', DWH.COD_GL_GROUP, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 
    'N/A', 'N/A', 'N/A', 'N/A', CAST(SUM(DWH.EOPBAL_CAP) AS DECIMAL(30,10)) AS AMO_EOPBAL, CAST(SUM(DWH.PL) AS DECIMAL(30,10)) AS AMO_PL, DWH.COD_CONT 
