...
SELECT COD_CONT, IDF_CTO, COD_GL, DES_GL, COD_ACCO_CENT, COD_OFFI, COD_VALUE, COD_CURRENCY, COD_ENTITY, COD_ACT_TYPE, CAST(AVG(FTP1) AS decimal(30, 6)) FTP1, CAST(AVG(FTP2) AS decimal(30, 6)) FTP2, IND_POOL, COD_INFO_SOURCE
FROM MIS_DWH_${var:tabla}
WHERE SUBSTR(DATA_DATE,1,6) = SUBSTR('${var:periodo}',1,6)f
AND IND_POOL = 'N'
AND (FTP1 <> 0 OR FTP2 <> 0)
AND COD_VALUE <> 'RSL'
GROUP BY COD_CONT, IDF_CTO, COD_GL, DES_GL, COD_ACCO_CENT, COD_OFFI, COD_VALUE, COD_CURRENCY, COD_ENTITY, COD_ACT_TYPE, IND_POOL, COD_INFO_SOURCE
;
*/
...
SELECT a.DATA_DATE, COD_CONT, a.IDF_CTO, COD_GL, DES_GL, a.COD_ACCO_CENT, a.COD_OFFI, 
    a.COD_VALUE, a.COD_CURRENCY, a.COD_ENTITY, a.COD_ACT_TYPE, 
    CASE WHEN ISNULL(ENG.COD_BCA_INT,ENG2.COD_BCA_INT) IN (4,5) AND (DAY(LAST_DAY(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'))) = 28 AND DAY(TO_TIMESTAMP(a.DATA_DATE, 'yyyyMMdd')) = 28)
         THEN SUM(b.EOPBAL_CAP * 3) 
    ELSE SUM(b.EOPBAL_CAP) END AS EOPBAL_CAP, 
    CASE WHEN ISNULL(ENG.COD_BCA_INT,ENG2.COD_BCA_INT) IN (4,5) AND (DAY(LAST_DAY(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'))) = 28 AND DAY(TO_TIMESTAMP(a.DATA_DATE, 'yyyyMMdd')) = 28)
...
INNER JOIN (SELECT DATA_DATE, IDF_CTO, COD_VALUE, SUM(EOPBAL_CAP) AS EOPBAL_CAP FROM MIS_DWH_${var:tabla}
...
SELECT DWH.COD_CONT, DWH.IDF_CTO, DWH.COD_GL, DWH.DES_GL, DWH.COD_ACCO_CENT, DWH.COD_OFFI, DWH.COD_BLCE_STATUS, DWH.COD_VALUE, DWH.COD_CURRENCY, DWH.COD_ENTITY, DWH.COD_PRODUCT, DWH.COD_SUBPRODUCT, DWH.COD_ACT_TYPE, 0 AS EOPBAL_CAP, 0 AS EOPBAL_INT, 0 AS AVGBAL_CAP, 0 AS AVGBAL_INT, 0 AS PL, 
    ...
    'UNIDAD_FONDEO' AS COD_BUSINESS_LINE, 'UNIDAD DE FONDEO' AS DES_BUSINESS_LINE, DWH.COD_METHOD1, DWH.FTP1, DWH.COD_METHOD2, DWH.FTP2,  --TODO: CONSULTAR CODIGO Y DESCRIPCION DE LINEA DE NEGOCIO PARA POOL DE FONDOS
