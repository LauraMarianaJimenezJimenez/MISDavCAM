...
--- Selecciona las variables del flujo contable u operacional, según corresponda ---         
    ...
    CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN 0 /*CAST((CONT.TOT_AVGBAL_CAP - OPER.TOT_AVGBAL_CAP) AS decimal(15, 2))*/ 
         WHEN CONT.DATA_DATE IS NOT NULL THEN CAST((CONT.TOT_AVGBAL_CAP) AS decimal(15, 2))
         WHEN OPER.DATA_DATE IS NOT NULL THEN CAST((-1 * OPER.TOT_AVGBAL_CAP) AS decimal(15, 2)) END AS AVGBAL_CAP, CASE WHEN OPER.DATA_DATE IS NOT NULL AND CONT.DATA_DATE IS NOT NULL THEN 0 /*CAST((CONT.TOT_AVGBAL_INT - OPER.TOT_AVGBAL_INT) AS decimal(15, 2))*/ 
...
FROM (
    SELECT TMP.DATA_DATE, '0' AS COD_ACCO_CENT, TMP.COD_CURRENCY, TMP.COD_ENTITY, TMP.COD_GL_GROUP, TMP.DES_GL_GROUP,
        TMP.ACCOUNT_CONCEPT, SUM(IFNULL(TMP.EOPBAL_CAP, 0) + IFNULL(TMP.EOPBAL_INT, 0)) AS TOT_EOPBAL_CAP, SUM(TMP.EOPBAL_INT) AS TOT_EOPBAL_INT, 
        SUM(TMP.AVGBAL_CAP) AS TOT_AVGBAL_CAP, SUM(TMP.AVGBAL_INT) AS TOT_AVGBAL_INT, SUM(TMP.PL) AS TOT_PL 
...
FULL JOIN (
    SELECT ACC.DATA_DATE, ACC.COD_ACCO_CENT, ACC.COD_CURRENCY, ACC.COD_ENTITY, ACC.COD_GL_GROUP, ACC.DES_GL_GROUP, 
        ACC.ACCOUNT_CONCEPT, SUM(IFNULL(ACC.EOPBAL_CAP, 0) + IFNULL(ACC.EOPBAL_INT, 0)) AS TOT_EOPBAL_CAP, SUM(ACC.EOPBAL_INT) AS TOT_EOPBAL_INT,
        SUM(ACC.AVGBAL_CAP) AS TOT_AVGBAL_CAP, SUM(ACC.AVGBAL_INT) AS TOT_AVGBAL_INT, SUM(ACC.PL) AS TOT_PL
...
--- código para saldo medios de partidas conciliatorias y cuentas netamente contables---
...
COD_PL_ACC, DES_PL_ACC, COD_BUSINESS_LINE, DES_BUSINESS_LINE, 
    CAST(SUM(b.AVGBAL_CAP) AS DECIMAL(30,10)), CAST(SUM(b.AVGBAL_INT) AS DECIMAL(30,10))
