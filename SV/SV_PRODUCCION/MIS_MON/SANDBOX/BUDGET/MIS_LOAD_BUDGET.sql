...
SELECT FROM_TIMESTAMP(LAST_DAY(TO_TIMESTAMP(A.DATA_DATE, 'yyyyMMdd')), 'yyyyMMdd') AS DATA_DATE, 
    ...
    A.EXP_TYPE,
    CAST(CASE
        WHEN A.COD_CURRENCY='COP' THEN CAST(A.PL AS decimal(30, 10)) / RT_PYG.EXCH_RATE
...
ON A.DATA_DATE = RT_BAL.DATA_DATE AND RT_BAL.COD_CONT = 'TC_FOTO' AND A.COD_CURRENCY = RT_BAL.COD_CURRENCY
AND IF(ISNULL(TRIM(RT_BAL.COD_ENTITY),'') = '' ,'1',A.COD_ENTITY) = IF(ISNULL(TRIM(RT_BAL.COD_ENTITY),'') = '','1',RT_BAL.COD_ENTITY)
LEFT JOIN MIS_PAR_EXCH_RATE RT_PYG
ON A.DATA_DATE = RT_PYG.DATA_DATE AND RT_PYG.COD_CONT = 'TC_FOTO' AND A.COD_CURRENCY = RT_PYG.COD_CURRENCY --DEBERIA SER PROMEDIO
AND IF(ISNULL(TRIM(RT_PYG.COD_ENTITY),'') = '' ,'1',A.COD_ENTITY) = IF(ISNULL(TRIM(RT_PYG.COD_ENTITY),'') = '','1',RT_PYG.COD_ENTITY)
