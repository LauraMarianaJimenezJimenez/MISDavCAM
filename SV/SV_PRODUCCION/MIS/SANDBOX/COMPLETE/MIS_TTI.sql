...
FROM 
    ...
    AND TMP.COD_PRODUCT = ANT.COD_PRODUCT AND TMP.COD_SUBPRODUCT = ANT.COD_SUBPRODUCT
    ...
    GROUP BY CONTEO, STRRIGHT(COD_INFO_SOURCE,2), TMP.COD_CONT, TMP.IDF_CTO, TMP.COD_GL, TMP.DES_GL, TMP.COD_ACCO_CENT, TMP.COD_OFFI, TMP.COD_VALUE, 
    TMP.COD_CURRENCY, TMP.COD_ENTITY, 
...
COMPUTE INCREMENTAL STATS MIS_TMP_${var:tabla}_SPE;
...
SELECT a.*,
    ...
    CASE WHEN a.FTP IS NOT NULL THEN 'N' --No se rechazan contratos con TTI especial
        ...
        WHEN (ISNULL(b.COD_CURVE,c.COD_CURVE) IS NULL OR cur.COD_CURVE IS NULL) THEN 'Y'
        ...
        ELSE 'N'  
    END AS REJECTION, CASE WHEN cur.COD_CURVE IS NULL THEN 'CUR' ELSE 'ENG' END AS COD_VAL
...
LEFT JOIN (SELECT DISTINCT COD_CURVE, COD_CURRENCY FROM MIS_PAR_CURVES WHERE DATA_DATE = '${var:periodo}') cur
ON ISNULL(b.COD_CURVE,c.COD_CURVE) = cur.COD_CURVE AND a.COD_CURRENCY = cur.COD_CURRENCY
;

----Inserción de registros rechazados y que no tendrán cálculo de TTI
COMPUTE INCREMENTAL STATS MIS_TMP_${var:tabla}_VAL;
INSERT OVERWRITE MIS_REJECTIONS_TTI PARTITION (DATA_DATE)   
SELECT DISTINCT a.COD_CONT, a.IDF_CTO, a.COD_BLCE_PROD, a.COD_BUSINESS_LINE, a.METHOD_TTI, a.COD_CURVE, a.COD_CURRENCY_TTI, a.COD_BCA_INT, 
    ...
    CASE WHEN COD_VAL = 'CUR' THEN 'SIN PARAMETRIZAR EN LA TABLA MIS_PAR_CURVES' ELSE 'SIN PARAMETRIZAR EN LA TABLA MIS_PAR_TTI_ENG' END, 
    a.DATA_DATE
...
SELECT ISNULL(PARAM_ST.METHOD_TTI,PARAM_ST2.METHOD_TTI) AS METHOD_TTI, ISNULL(PARAM_ST.COD_CURVE,PARAM_ST2.COD_CURVE) AS COD_CURVE, 
    ...
    CASE
       ...
       THEN ROUND(DATEDIFF(TO_TIMESTAMP(SALIDA_DWH.FINISH_DATE,'yyyyMMdd'), TO_TIMESTAMP(ISNULL(SALIDA_DWH.START_DATE_2,SALIDA_DWH.START_DATE),'yyyyMMdd')) * 
        ISNULL(PARAM_ST.TERM_FACTOR,PARAM_ST2.TERM_FACTOR),0)
        WHEN ISNULL(PARAM_ST.METHOD_TTI,PARAM_ST2.METHOD_TTI) = 'ST'
        THEN DATEDIFF(TO_TIMESTAMP(SALIDA_DWH.FINISH_DATE,'yyyyMMdd'), TO_TIMESTAMP(ISNULL(SALIDA_DWH.START_DATE_2,SALIDA_DWH.START_DATE),'yyyyMMdd'))
        ELSE NULL
    END AS NUMDIAS
FROM
    (SELECT DWH.*, 
    CASE WHEN DWH.COD_RATE_TYPE = 'A' AND DWH.DATE_ORIGIN = DWH.DATA_DATE THEN DWH.DATE_ORIGIN
        WHEN (DWHP.IDF_CTO IS NULL AND ISNULL(DWH.DATE_ORIGIN,'0') < ISNULL(DWH.DATA_DATE,'0')) THEN DWH.DATE_ORIGIN
    ...
    CASE WHEN DT_PREV.COD_RATE_TYPE = 'V' AND DWH.COD_RATE_TYPE = 'F' THEN DWH.DATA_DATE 
         WHEN DWHP.EOPBAL_CAP < 0 AND DWH.EOPBAL_CAP >= 0 THEN DWH.DATA_DATE
         ELSE NULL END AS START_DATE_2,
    ...
    LEFT JOIN (SELECT DATA_DATE, IDF_CTO, COD_VALUE, COD_ENTITY, SUM(EOPBAL_CAP) AS EOPBAL_CAP FROM MIS_DWH_${var:tabla} 
               WHERE TO_TIMESTAMP(DATA_DATE, 'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP('${var:periodo}','yyyyMMdd'), 1)
               GROUP BY DATA_DATE, IDF_CTO, COD_VALUE, COD_ENTITY) DWHP
    ...
    WHERE DWH.FTP IS NULL 
        ...
        OR (DWHP.EOPBAL_CAP < 0 AND DWH.EOPBAL_CAP >= 0)
        )
    ) SALIDA_DWH
LEFT JOIN 
...
SELECT a.COD_BLCE_PROD, a.COD_BUSINESS_LINE, a.COD_BLCE_STATUS, a.COD_SEGMENT, a.COD_AMRT_MET, a.COD_CURRENCY, a.METHOD_TTI, a.COD_CURVE, a.TERM, a.AVG_PERIOD, 
...
GROUP BY a.COD_BLCE_PROD, a.COD_BUSINESS_LINE, a.COD_BLCE_STATUS, a.COD_SEGMENT,a.COD_AMRT_MET, a.COD_CURRENCY, a.METHOD_TTI, a.COD_CURVE, a.TERM, a.AVG_PERIOD, a.IND_VOLATILITY, a.PERC_VOLATILITY;
...
SELECT a.DATA_DATE, a.COD_CONT, a.IDF_CTO, a.COD_GL, a.DES_GL, a.COD_ACCO_CENT, a.COD_OFFI, a.COD_BLCE_STATUS, a.COD_VALUE, a.COD_CURRENCY, a.COD_ENTITY, 
    ...
    a.DATE_ORIGIN, a.EXP_DATE, a.DATE_LAST_REV, b.METHOD_TTI, a.COD_SEGMENT, a.COD_AMRT_MET
...
COMPUTE INCREMENTAL STATS MIS_TMP_${var:tabla}_STERM2;
...
SELECT ISNULL(PAR_LIQ.COD_CURVE_LIQ,PAR_LIQ2.COD_CURVE_LIQ) AS COD_CURVE, ISNULL(PAR_LIQ.IND_SPREAD,PAR_LIQ2.IND_SPREAD) AS IND_SPREAD, DWH.COD_CONT, DWH.IDF_CTO, DWH.COD_GL, DWH.DES_GL, DWH.COD_ACCO_CENT, DWH.COD_OFFI, DWH.COD_BLCE_STATUS, DWH.COD_VALUE, DWH.COD_CURRENCY, 
    ...
    DWH.DES_BLCE_PROD, DWH.COD_BUSINESS_LINE, DWH.DES_BUSINESS_LINE, CASE WHEN DT.CARD_NUMBER IS NOT NULL AND DWH.EOPBAL_CAP < 0 THEN 'NEG' ELSE 'LIQ' END AS COD_METHOD1, CAST(CASE WHEN DT.CARD_NUMBER IS NOT NULL AND DWH.EOPBAL_CAP < 0 THEN 0 ELSE ISNULL(TTI.RATE_LIQ,CAST(PAS_TTI.COD_CURVE_LIQ AS DECIMAL(30,10)) * IF(PAS_TTI.IND_VOLATILITY = 'Y', PAS_TTI.PERC_VOLATILITY, 1)) END AS decimal(30, 10)) AS FTP, DWH.FTP_RESULT, DWH.IND_POOL,  
...
FROM
    ...
    WHERE INI.COD_CONT IN ('PLZ','CTA') OR (INI.COD_CONT NOT IN ('CTA','PLZ') AND INI.COD_RATE_TYPE IN ('V','A'))
    ) DWH
LEFT JOIN (SELECT * FROM MIS_PAR_TTI_ENG WHERE METHOD_LIQ = 'PAS_LIQ') PAS_TTI
ON PAS_TTI.COD_BLCE_PROD=DWH.COD_BLCE_PROD AND PAS_TTI.COD_BLCE_STATUS=DWH.COD_BLCE_STATUS AND PAS_TTI.COD_CURRENCY=DWH.COD_CURRENCY
    AND IF(TRIM(PAS_TTI.COD_BUSINESS_LINE)='', '', DWH.COD_BUSINESS_LINE) = TRIM(PAS_TTI.COD_BUSINESS_LINE) 
    AND IF(TRIM(PAS_TTI.COD_SEGMENT)='', '', DWH.COD_SEGMENT) = TRIM(PAS_TTI.COD_SEGMENT)
    AND (IF(DWH.COD_AMRT_MET IS NULL OR TRIM(DWH.COD_AMRT_MET) = '', 'NA', DWH.COD_AMRT_MET)=
    IF(PAS_TTI.COD_AMRT_MET IS NULL OR TRIM(PAS_TTI.COD_AMRT_MET) = '', 'NA', PAS_TTI.COD_AMRT_MET))
LEFT JOIN 
...
--COMPUTE INCREMENTAL STATS MIS_TMP_${var:tabla}_LPE;
...
ON TO_TIMESTAMP(DT_PREV.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP(LPE.DATA_DATE, 'yyyyMMdd'), 1) AND 
    DT_PREV.IDF_CTO = LPE.IDF_CTO AND DT_PREV.COD_ENTITY = LPE.COD_ENTITY 
LEFT JOIN (
    SELECT DATA_DATE, IDF_CTO, COD_VALUE, COD_ENTITY, SUM(EOPBAL_CAP) AS EOPBAL_CAP FROM MIS_DWH_${var:tabla} 
    WHERE TO_TIMESTAMP(DATA_DATE, 'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP('${var:periodo}','yyyyMMdd'), 1)
    GROUP BY DATA_DATE, IDF_CTO, COD_VALUE, COD_ENTITY) DWHP
...
ON LPE.COD_CURVE=CUR.COD_CURVE AND LPE.COD_CURRENCY=CUR.COD_CURRENCY 
     AND (LPE.DATE_ORIGIN = LPE.DATA_DATE OR (DT_PREV.COD_RATE_TYPE = 'F' AND LPE.COD_RATE_TYPE = 'V') 
          ...
          OR (DWHP.EOPBAL_CAP < 0 AND LPE.EOPBAL_CAP >= 0)
    ) AND 
    (CASE WHEN LPE.COD_RATE_TYPE = 'A' AND LPE.DATE_ORIGIN = LPE.DATA_DATE THEN LPE.DATE_ORIGIN 
        WHEN DWHP.EOPBAL_CAP < 0 AND LPE.EOPBAL_CAP >= 0 THEN LPE.DATA_DATE
        WHEN (DWHP.IDF_CTO IS NULL AND ISNULL(LPE.DATE_ORIGIN,'0') < ISNULL(LPE.DATA_DATE,'0')) THEN LPE.DATE_ORIGIN
...
--COMPUTE INCREMENTAL STATS MIS_TMP_${var:tabla}_UPD;
...
SELECT DWH.DATA_DATE, DWH.COD_CONT, DWH.IDF_CTO, DWH.COD_GL, DWH.DES_GL, DWH.COD_ACCO_CENT, DWH.COD_OFFI, DWH.COD_BLCE_STATUS, DWH.COD_VALUE, DWH.COD_CURRENCY, DWH.COD_ENTITY, 
    DWH.COD_PRODUCT, DWH.COD_SUBPRODUCT, DWH.COD_ACT_TYPE, DWH.EOPBAL_CAP, DWH.EOPBAL_INT, DWH.AVGBAL_CAP, DWH.AVGBAL_INT, DWH.PL, DWH.COD_INFO_SOURCE, 
...
LEFT JOIN (
     ...
     TO_TIMESTAMP(TMP.DATA_DATE,'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd'), 1) AND IND_POOL = 'N')  DM --Propagar valor del día anterior
ON TO_TIMESTAMP(DM.DATA_DATE, 'yyyyMMdd') = DATE_SUB(TO_TIMESTAMP(DWH.DATA_DATE,'yyyyMMdd'), 1) 
    AND DM.COD_CONT=DWH.COD_CONT AND DM.IDF_CTO=DWH.IDF_CTO 
...
COMPUTE INCREMENTAL STATS MIS_TMP_${var:tabla}_FTP;
...
COMPUTE INCREMENTAL STATS MIS_TMP_${var:tabla}_FINAL;
