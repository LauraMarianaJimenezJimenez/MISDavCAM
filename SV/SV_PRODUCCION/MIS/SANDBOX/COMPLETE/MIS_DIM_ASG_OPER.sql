...
SELECT a.*, CASE WHEN a.COD_GL <> c.COD_GL_GROUP THEN c.COD_GL_GROUP ELSE CASE WHEN a.COD_GL IS NOT NULL THEN a.COD_GL ELSE B.COD_GL_GROUP END END AS COD_GL_GROUP, CASE WHEN a.COD_GL IS NOT NULL THEN d.DES_GL_GROUP ELSE b.COD_GL_GROUP END AS DES_GL_GROUP, b.ACCOUNT_CONCEPT 
FROM (
    SELECT a.* 
    FROM MIS_APR_${var:tabla} a
    WHERE a.DATA_DATE = '${var:periodo}') a
LEFT JOIN (
    SELECT DISTINCT a.COD_ENTITY, a.COD_PRODUCT, a.COD_SUBPRODUCT, a.COD_CURRENCY, a.COD_VALUE, a.COD_GL_GROUP, b.DES_GL_GROUP, a.COD_BLCE_STATUS, b.ACCOUNT_CONCEPT
    FROM MIS_PAR_REL_CAF_OPER a    
    LEFT JOIN MIS_PAR_CAT_CAF b
    ON a.COD_GL_GROUP=b.COD_GL_GROUP AND a.COD_CURRENCY=b.COD_CURRENCY
) b   
...
--- El Salvador ---
LEFT JOIN MIS_PAR_REL_CAF_ACC c
ON a.COD_GL=c.COD_GL AND a.COD_ENTITY=c.COD_ENTITY AND a.COD_CURRENCY=c.COD_CURRENCY
LEFT JOIN MIS_PAR_CAT_CAF d
ON a.COD_GL = d.COD_GL_GROUP AND a.COD_CURRENCY=d.COD_CURRENCY
;
...
INSERT OVERWRITE MIS_REJECTIONS_DIMENSIONS (DIM_REJ, COD_ENTITY, COD_CURRENCY, COD_GL, COD_GL_GROUP, COD_ACT_TYPE, COD_PRODUCT, COD_SUBPRODUCT, COD_RATE_TYPE, COD_BLCE_STATUS, COD_VALUE, 
    COD_BLCE_PROD, COD_MANAGER, TYP_DOC_ID, AMO_EOPBAL, AMO_PL)  
...
SELECT 'AGRUPADOR CONTABLE' AS DIM_REJ, a.COD_ENTITY, a.COD_CURRENCY, 'N/A', 'N/A', 'N/A', a.COD_PRODUCT, a.COD_SUBPRODUCT, 'N/A', a.COD_BLCE_STATUS, a.COD_VALUE, 
    'N/A', 'N/A', 'N/A', CAST(SUM(IFNULL(a.EOPBAL_CAP, 0) + IFNULL(a.EOPBAL_INT, 0)) AS DECIMAL(30,10)) AS AMO_EOPBAL, CAST(SUM(a.PL) AS DECIMAL(30,10)) AS AMO_PL, a.DATA_DATE, a.COD_CONT 
...
SELECT a.*, CASE WHEN b.IND_DETAIL = 'Y' THEN CONCAT(b.COD_PL_ACC,'/',ISNULL(e.COD_BLCE_PROD,'')) ELSE b.COD_PL_ACC END COD_PL_ACC, c.DES_PL_ACC 
FROM (
    SELECT a.*
    FROM MIS_TMP_STG_${var:tabla}_1 a
...
SELECT a.*, d.COD_BUSINESS_LINE, e.DES_BUSINESS_LINE, b.IDF_CLI, c.COD_TYP_CLNT  
FROM (
    SELECT a.*
    FROM MIS_TMP_STG_${var:tabla}_3 a
...
ON IF(c.COD_TYP_CLNT IS NULL OR c.COD_TYP_CLNT = '', 'NA', c.COD_TYP_CLNT) = IF(d.COD_TYP_CLNT IS NULL OR d.COD_TYP_CLNT = '', 'NA', d.COD_TYP_CLNT) AND 
    IF(a.COD_BLCE_PROD IS NULL OR a.COD_BLCE_PROD = '', 'NA', a.COD_BLCE_PROD) = IF(d.COD_BLCE_PROD IS NULL OR d.COD_BLCE_PROD = '', 'NA', d.COD_BLCE_PROD)
LEFT JOIN MIS_PAR_CAT_BL e
ON e.COD_BUSINESS_LINE = d.COD_BUSINESS_LINE 
; 
...
SELECT 'LINEA DE NEGOCIO' AS DIM_REJ, 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 
    a.COD_BLCE_PROD, 'N/A', a.COD_TYP_CLNT, CAST(SUM(IFNULL(a.EOPBAL_CAP, 0) + IFNULL(a.EOPBAL_INT, 0)) AS DECIMAL(30,10)) AS AMO_EOPBAL, CAST(SUM(a.PL) AS DECIMAL(30,10)) AS AMO_PL, a.DATA_DATE, a.COD_CONT 
...
SELECT 
    a.COD_CONT, a.IDF_CTO, a.COD_GL, a.DES_GL, a.COD_ACCO_CENT, a.COD_OFFI, a.COD_BLCE_STATUS, a.COD_VALUE, a.COD_CURRENCY, 
    a.COD_ENTITY, a.COD_PRODUCT, a.COD_SUBPRODUCT, a.COD_ACT_TYPE, a.EOPBAL_CAP, a.EOPBAL_INT, a.AVGBAL_CAP, a.AVGBAL_INT, 
    a.PL, a.COD_GL_GROUP, a.DES_GL_GROUP, a.ACCOUNT_CONCEPT,  
