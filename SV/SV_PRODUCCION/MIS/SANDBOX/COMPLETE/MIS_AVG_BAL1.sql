...
0 AS AVGBAL_CAP, 0 AS AVGBAL_INT, CASE WHEN COD_CONT <> 'PROV' THEN MAX(PL) ELSE 0 END AS PL1, COD_INFO_SOURCE
FROM MIS_APR_${var:tabla}
WHERE --STRRIGHT(COD_INFO_SOURCE,2) <> '_M' AND
IDF_CTO <> '2204030000010060000100000000' AND
...
GROUP BY COD_CONT, IDF_CTO, COD_GL, DES_GL, COD_ACCO_CENT, COD_OFFI, COD_BLCE_STATUS, COD_VALUE, COD_CURRENCY, COD_ENTITY, COD_PRODUCT, COD_SUBPRODUCT, COD_ACT_TYPE, 
COD_INFO_SOURCE) A
LEFT JOIN
...
ON A.IDF_CTO = B.IDF_CTO AND A.COD_BLCE_STATUS = B.COD_BLCE_STATUS AND A.COD_VALUE = B.COD_VALUE AND A.COD_PRODUCT = B.COD_PRODUCT AND STRLEFT(A.COD_SUBPRODUCT,7) = STRLEFT(B.COD_SUBPRODUCT,7) AND A.COD_CONT = B.COD_CONT
...
FROM MIS_APR_${var:tabla}
WHERE IDF_CTO <> '2204030000010060000100000000' AND
(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd') = DAYS_SUB(TRUNC(TO_TIMESTAMP('${var:periodo}','yyyyMMdd'),'MM'),1))
...
INSERT INTO MIS_APR_CONTRACT_DT PARTITION(DATA_DATE, COD_INFO_SOURCE)
    ...
    PREDEF_RATE_IND, PREDEF_RATE, IND_CHANNEL, COD_TYP_LIC, COU_CAR_OFF, COD_CONV, COD_EXEC_CTO, COD_COVID_PORT, FIELD1_CTO, FIELD2_CTO, FIELD3_CTO, FIELD4_CTO, FIELD5_CTO, FIELD6_CTO, FIELD7_CTO, FIELD8_CTO, FIELD9_CTO, FIELD10_CTO, DATE_DISB, CARD_NUMBER, COD_PROG_CARD, DES_PROG_CARD, '${var:periodo}' AS DATA_DATE, COD_INFO_SOURCE 
    ...
    INNER JOIN 
        (SELECT ARR.FEC_MAX, ARR.IDF_CTO AS IDF_CTO_ARR FROM 
            (SELECT ANT.*  FROM 
                ...
                WHERE IDF_CTO <> '2204030000010060000100000000' AND /*YEAR(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = YEAR(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd')) 
                AND*/ ((MONTH(TO_TIMESTAMP(DATA_DATE,'yyyyMMdd')) = MONTH(TO_TIMESTAMP('${var:periodo}', 'yyyyMMdd')) 
